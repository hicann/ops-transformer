...
namespace ops {
class CompressorV2 : public OpDef {
public:
    ...
    explicit CompressorV2(const char *name) : OpDef(name)
    {
        this->Input("x")
            .ParamType(REQUIRED)
        ...
        // V2 仅保留 arch35（950）实现，不再注册 A2/A3
        this->AICore().AddConfig("ascend950", aicore_config);
    }
};
OP_ADD(CompressorV2, optiling::CompressorV2CompileInfo);
} // namespace ops
