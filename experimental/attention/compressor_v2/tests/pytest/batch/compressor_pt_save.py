...
import ast
import argparse
# import custom_ops  # unused, commented to avoid ImportError
        ...
        for _, row in df.iterrows():
            test_cases.append(
                (
                    ...
                    bool(row.get("is_contiguous", False)),
                )
            )
...
def compressor_output_single(data_case):
    ...
    if cache_mode == 1:
        ...
        # 直接用传入的 block_size 构造 state_cache，不做约束裁剪
        if batch_size == 0:
            kv_state = torch.tensor(
                np.random.uniform(
                    kv_state_datarange[0],
                    kv_state_datarange[1],
                    (batch_size, block_size, coff * head_dim),
                )
            ).to(torch.float32)
            score_state = torch.tensor(
                np.random.uniform(
