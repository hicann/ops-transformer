...
def extract_compressor_row(prof_folder):
    ...
    target = df[df["Op Name"] == "Compressor"]
    if target.empty:
        return None
...
def collect_and_save_incremental(prof_folder, test_result_path):
    ...
    row_data = extract_compressor_row(prof_folder)
    if row_data is None:
        return None

    last_idx = df_result.shape[0] - 1
    case_name = df_result.iloc[last_idx]["Testcase_Name"]
...
def collect_all(
    ...
    for i in range(df_b.shape[0]):
        ...
        case_name = df_b.iloc[i]["Testcase_Name"]
        row_data = extract_compressor_row(prof)
        ...
        if row_data is not None:
            ...
            print(f"  [{prof_idx + 1}] {case_name}: 未找到Compressor数据")
    ...
    if is_compare:
        ...
        for i in perf_rows.keys():
            try:
                ...
                df_b.at[i, "compressor_perf_diff"] = sas_diff

                if abs(sas_diff) > perf_threshold:
                    df_b.at[i, "perf_result"] = "Failed"
                    perf_fail_list.append(df_b.iloc[i]["Testcase_Name"])
                else:
