...
public:
    explicit MinimaxSparseAttentionSplitKv(const char *name)
        : OpDef(name)
    {
        this->Input("query").ParamType(REQUIRED).DataType({ge::DT_BF16}).FormatList({ge::FORMAT_ND});
        this->Input("key").ParamType(REQUIRED).DataType({ge::DT_BF16}).FormatList({ge::FORMAT_ND});
        this->Input("value").ParamType(REQUIRED).DataType({ge::DT_BF16}).FormatList({ge::FORMAT_ND});
        // Optional: present => paged KV cache (4D key/value + block table).
        // Absent  => contiguous dense K/V matching query + inputLayout
        // (TND / BNSD / BSND).
        this->Input("blockTable").ParamType(OPTIONAL).DataType({ge::DT_INT32}).FormatList({ge::FORMAT_ND});
        ...
        this->Input("k2qRowPtr").ParamType(REQUIRED).DataType({ge::DT_INT32}).FormatList({ge::FORMAT_ND});
        this->Input("k2qQIndices").ParamType(REQUIRED).DataType({ge::DT_INT32}).FormatList({ge::FORMAT_ND});
        this->Input("k2qSlotIndices").ParamType(REQUIRED).DataType({ge::DT_INT32}).FormatList({ge::FORMAT_ND});
        this->Input("actualSeqLengths").ParamType(REQUIRED).DataType({ge::DT_INT32}).FormatList({ge::FORMAT_ND});
        this->Input("actualSeqLengthsKv").ParamType(REQUIRED).DataType({ge::DT_INT32}).FormatList({ge::FORMAT_ND});
        this->Output("attentionOut").ParamType(REQUIRED).DataType({ge::DT_BF16}).FormatList({ge::FORMAT_ND});
        ...
        this->Output("softmaxLse").ParamType(REQUIRED).DataTypeList({ge::DT_FLOAT}).FormatList({ge::FORMAT_ND});
