...
#ifndef VF_SOFTMAX_DN_CAST_NZ_MXFP4_QS64_KVS256_MASK_H_
#define VF_SOFTMAX_DN_CAST_NZ_MXFP4_QS64_KVS256_MASK_H_
                                                                 ...
                                                                 const bool bAllInvalid)
{
    ...
    for (uint16_t g = 0; g < validGroups; ++g) {
        ...
        // 更新当前块最大值，用于下一次循环
        if constexpr (MX_QUANT_MODE == MXQuantMode::OCP) {
            Adds(next_group_max, next_group_max, NEG_TWO_VALE, preg_VL64_16bit);
        }
...
template <MXQuantMode MX_QUANT_MODE = MXQuantMode::OCP, bool clear_gmax, typename T, typename T2, uint16_t QsBase = 128,
          uint16_t chunkY = 0>
__simd_vf__ inline void softmax_with_group_max_qs64_kvs256_chunk_vf(__ubuf__ T2 *p_dest, __ubuf__ T *s,
                                                                    __ubuf__ T *local_group_max, __ubuf__ T *global_max,
                                                                    __ubuf__ uint8_t *indexes_ub, const T NEG_LOG2_CX,
                                                                    const uint16_t effY)
{
    softmax_with_group_max_qs64_chunk_vf<MX_QUANT_MODE, clear_gmax, T, T2, QsBase>(
        p_dest, s, local_group_max, global_max, indexes_ub, NEG_LOG2_CX, chunkY, effY, false);
}

template <MXQuantMode MX_QUANT_MODE = MXQuantMode::OCP, bool clear_gmax, typename T, typename T2, uint16_t QsBase = 128,
          uint16_t chunkY = 0>
__aicore__ inline void SoftmaxWithGroupMaxQs64Kvs256ChunkCallVF(
    ...
    const LocalTensor<T> &globalMax, const LocalTensor<uint8_t> &indexesBuf, const T NEG_LOG2_CX, const uint16_t effY)
{
    ...
    softmax_with_group_max_qs64_kvs256_chunk_vf<MX_QUANT_MODE, clear_gmax, T, T2, QsBase, chunkY>(
        p_dest, s_ub, local_group_max, global_max, indexes_ub, NEG_LOG2_CX, effY);
}

} // namespace NpuArch::Epilogue::Block::Mxfp4VF
#endif // VF_SOFTMAX_DN_CAST_NZ_MXFP4_QS64_KVS256_MASK_H_
