...
#ifndef W8A8_KERNEL_COMMON_HPP
#define W8A8_KERNEL_COMMON_HPP
...
struct GenericBlockSparseAttentionTilingData {
    ...
    uint32_t vL1BufNum;
    uint32_t pL1BufNum;
    // Paged KV page base stride in elements (dim0). PA_NZ uses the same
    // element-count page stride while its inner layout is [D/C0, block, C0].
    uint64_t kStride0;
    uint64_t vStride0;
    ...
    uint32_t antiquantFlag;
    uint32_t msdIterNum;
    uint64_t queryPreProcessSize;
    uint64_t amaxQSize;
    uint64_t amaxPSize;
    uint32_t headSplitFactor;
};
