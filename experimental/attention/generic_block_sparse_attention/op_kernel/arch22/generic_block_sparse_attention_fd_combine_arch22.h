...
#ifndef GENERIC_BLOCK_SPARSE_ATTENTION_FD_COMBINE_ARCH22_H
#define GENERIC_BLOCK_SPARSE_ATTENTION_FD_COMBINE_ARCH22_H
...
class GenericBlockSparseAttentionFdCombineArch22 {
public:
    ...
    __aicore__ inline GenericBlockSparseAttentionFdCombineArch22(Resource &resource)
    {
    ...
    __aicore__ inline void operator()(__gm__ const GsaMetadata::Metadata *meta,
                                      __gm__ GenericBlockSparseAttn::GenericBlockSparseAttentionTilingData *tilingData,
                                      AscendC::GlobalTensor<float> &gPartialLse,
                                      AscendC::GlobalTensor<float> &gPartialO, AscendC::GlobalTensor<ElementO> &gO,
                                      CuSeqTensor &gCuSeqLengths, SeqUsedTensor &gSeqUsedQ, bool hasSeqUsedQ)
    {
        ...
        const uint32_t combineTaskNum = static_cast<uint32_t>(meta->combineTaskNum);
        ...
        for (uint32_t work = aivIdx; work < workCount; work += aivNum) {
            ...
            const __gm__ GsaMetadata::CombineSchedule &schedule = meta->combineSchedules[combineTask];
            const uint32_t baseTask = static_cast<uint32_t>(schedule.baseTask);
            const uint32_t splitCount = static_cast<uint32_t>(schedule.partialCount);
            const uint32_t splitCountAlign = RoundUp(splitCount, FLOATS_PER_BLOCK);
            const uint32_t firstPartialTask = static_cast<uint32_t>(schedule.partialStart);

            PrepareWeights(gPartialLse, firstPartialTask, splitCount, splitCountAlign, sourceSubBlock, sourceLocalRow,
                           rowCount, rowCountAlign, lseSubStride, lsePartialStride);
            ...
            uint32_t kvHead = 0U;
            if (!GsaFd::DecodeTaskStorage(baseTask, tilingData->kvHeads, tilingData->batch, gCuSeqLengths, gSeqUsedQ,
                                          hasSeqUsedQ, qStorageToken, qTokenInBatch, batchIdx, kvHead)) {
                continue;
            }
            const uint64_t baseOutputOffset =
                (static_cast<uint64_t>(qStorageToken) * tilingData->numHeads + kvHead * groupSize + groupRowStart) *
                HEAD_DIM;
...
private:
    __aicore__ inline void PrepareWeights(AscendC::GlobalTensor<float> &gPartialLse, uint32_t firstPartialTask,
                                          uint32_t splitCount, uint32_t splitCountAlign, uint32_t sourceSubBlock,
                                          uint32_t sourceLocalRow, uint32_t rowCount, uint32_t rowCountAlign,
                                          uint32_t lseSubStride, uint32_t lsePartialStride)
    {
        ...
        const uint64_t lseOffset =
            static_cast<uint64_t>(firstPartialTask) * lsePartialStride + sourceSubBlock * lseSubStride + sourceLocalRow;
...
#endif // GENERIC_BLOCK_SPARSE_ATTENTION_FD_COMBINE_ARCH22_H
