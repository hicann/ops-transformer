...
public:
    ...
    __aicore__ inline void operator()(AscendC::GlobalTensor<ElementA> gA, AscendC::GlobalTensor<ElementB> gB,
                                      ...
                                      uint32_t &blockSize, uint32_t kvSeqlen, uint64_t strideKV, uint32_t blockStackNum,
                                      Arch::CrossCoreFlag softmaxFlag, uint32_t &y, uint32_t selectNum,
                                      uint32_t kvYBlockNum, const int32_t *physicalBlockIds, bool usePhysicalBlockIds)
    {
        ...
        // load V
        // int8 V needs 2*C0 row padding in L1 (fractal alignment).
        uint32_t l1BRows = stackSeqTile;
        if constexpr (std::is_same_v<ElementB, int8_t>) {
            l1BRows = RoundUp<2 * C0_NUM_PER_FRACTAL>(stackSeqTile);
        }
        LayoutBInL1 layoutBInL1 = LayoutBInL1::template MakeLayout<ElementB>(l1BRows, embed);
        ...
        for (uint32_t blockStackIdx = 0; blockStackIdx < nL1Loop; ++blockStackIdx) {
            ...
            uint32_t currentYIdx = usePhysicalBlockIds ? static_cast<uint32_t>(physicalBlockIds[currentSelectYIdx]) :
                                                         static_cast<uint32_t>(gSelectIdx.GetValue(currentSelectYIdx));
            uint32_t offsetInKV = currentYIdx * y + currentYoffset;

            while (processSize < kActual && currentSelectYIdx < selectNum &&
                   (usePhysicalBlockIds || (currentYIdx < kvYBlockNum && offsetInKV < kvSeqlen))) {
                uint32_t yAcutal =
                    usePhysicalBlockIds ?
                        y :
                ...
                if constexpr (DispatchPolicy::PAGED_CACHE_FLAG) {
                    // PA_NZ cache is laid out as [D/C0, blockSize, C0].
                    // Keep the cache in NZ form and gather the token interval
                    // directly into the raw L1 tile expected by the PV Cube.
                    constexpr uint32_t blockElementCnt = BYTE_PER_C0 / sizeof(ElementB);
                    AscendC::DataCopyParams nzParams;
                    nzParams.blockLen = actualYSize;
                    nzParams.blockCount = CeilDiv(nActual, blockElementCnt);
                    nzParams.srcStride = blockSize - actualYSize;
                    nzParams.dstStride = RoundUp<C0_NUM_PER_FRACTAL>(stackSeqTile) - actualYSize;
                    AscendC::DataCopy(l1BTile,
                                      gB[static_cast<uint64_t>(currentYIdx) * strideKV +
                                         static_cast<uint64_t>(currentYoffset) * blockElementCnt],
                                      nzParams);
                } else {
                ...
                if (currentYoffset >= yAcutal) {
                    ...
                    currentYoffset = 0;
                    currentYIdx = usePhysicalBlockIds ? static_cast<uint32_t>(physicalBlockIds[currentSelectYIdx]) :
                                                        static_cast<uint32_t>(gSelectIdx.GetValue(currentSelectYIdx));
        ...
        Arch::CrossCoreWaitFlag<0x2>(softmaxFlag);
