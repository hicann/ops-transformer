...
public:
                                   ...
                                   uint32_t &singleGroupHeads, uint64_t qStride)
    {
        ...
        copyGmToL1A(l1ATensor, gA, layoutAInL1, layoutSingleANd, tokenNumPerGroup, qStride, 1, layoutAInL1.stride(1),
                    rowNumRound);
    ...
    __aicore__ inline void operator()(AscendC::GlobalTensor<ElementA> gA, AscendC::GlobalTensor<ElementB> gB,
                                      ...
                                      uint32_t &blockSize, uint64_t strideKV, uint32_t &y, uint32_t selectNum,
                                      uint32_t kvYBlockNum, uint32_t &kvSeqlen, const int32_t *physicalBlockIds,
                                      bool usePhysicalBlockIds)
    {
        ...
        tileNNumPerPaged = tileNNumPerPaged == 0 ? 1 : tileNNumPerPaged;
        ...
        for (uint32_t nL1Idx = 0; nL1Idx < nL1Loop; ++nL1Idx) {
            ...
            while (processSize < nActual && currentSelectYIdx < selectNum &&
                ...
                if constexpr (DispatchPolicy::PAGED_CACHE_FLAG) {
                    ...
                    // Each d-fractal contains one contiguous C0-wide value
                    // for every token, so a 2-D DataCopy gathers the selected
                    // token interval without an ND materialization.
                    ...
                    nzParams.blockCount = CeilDiv(kActual, blockElementCnt);
                    nzParams.srcStride = blockSize - actualYSize;
                    nzParams.dstStride = RoundUp<C0_NUM_PER_FRACTAL>(nActual) - actualYSize;
                    AscendC::DataCopy(l1BTensor[l1KvPingPongFlag][processSize * blockElementCnt],
