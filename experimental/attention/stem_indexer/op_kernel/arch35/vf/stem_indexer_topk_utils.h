                                                 ...
                                                 const uint32_t padEnd)
{
    ...
    Reg::MaskReg maskAll = AscendC::Reg::CreateMask<SCORE_T, AscendC::Reg::MaskPattern::ALL>();
    Reg::MaskReg maskGeStart;
    Reg::MaskReg maskLtEnd;
    Reg::MaskReg maskPad;
    Reg::RegTensor<IDX_T> regIdx;
    Reg::RegTensor<SCORE_T> regOldValue;
    Reg::RegTensor<SCORE_T> regZero;
    Reg::RegTensor<SCORE_T> regOutValue;
    Reg::Duplicate(regZero, (SCORE_T)0, maskAll);

    for (uint32_t blockBase = blockStart; blockBase < blockEnd; blockBase += VF_ELEMS) {
        Reg::Arange(regIdx, static_cast<IDX_T>(blockBase));
        Reg::LoadAlign<SCORE_T, Reg::LoadDist::DIST_NORM>(regOldValue, rowBuf + blockBase);
        Reg::Compares<SCORE_T, CMPMODE::GE>(maskGeStart, (Reg::RegTensor<SCORE_T> &)regIdx, (SCORE_T)padStart, maskAll);
        Reg::Compares<SCORE_T, CMPMODE::LT>(maskLtEnd, (Reg::RegTensor<SCORE_T> &)regIdx, (SCORE_T)padEnd, maskAll);
        Reg::And(maskPad, maskGeStart, maskLtEnd, maskAll);
        Reg::Select(regOutValue, regZero, regOldValue, maskPad);
        Reg::StoreAlign<SCORE_T, Reg::StoreDist::DIST_NORM>(rowBuf + blockBase, regOutValue, maskAll);
    }
}
...
template <typename SCORE_T>
__simd_vf__ inline void PadMrgFourRowsFirstS2RangeVFImpl(__ubuf__ SCORE_T *mrgValueBuf, const uint32_t rowStride,
                                                         const uint32_t rowIdx0, const uint32_t rowIdx1,
                                                         const uint32_t rowIdx2, const uint32_t rowIdx3,
                                                         const uint32_t topkNum0, const uint32_t topkNum1,
                                                         const uint32_t topkNum2, const uint32_t topkNum3,
                                                         const uint32_t s2ValidLen0, const uint32_t s2ValidLen1,
...
template <typename SCORE_T>
__simd_vf__ inline void PadMrgFourRowsNotFirstS2RangeVFImpl(__ubuf__ SCORE_T *mrgValueBuf, const uint32_t rowStride,
                                                            const uint32_t rowIdx0, const uint32_t rowIdx1,
                                                            const uint32_t rowIdx2, const uint32_t rowIdx3,
                                                            const uint32_t topkNum0, const uint32_t topkNum1,
                                                            const uint32_t topkNum2, const uint32_t topkNum3,
                                                  ...
                                                  const uint32_t initialBlocks)
{
    using IDX_T = std::conditional_t<std::is_same_v<SCORE_T, uint16_t>, int16_t, int32_t>;
    Reg::MaskReg maskAll = AscendC::Reg::CreateMask<SCORE_T, AscendC::Reg::MaskPattern::ALL>();
    Reg::MaskReg maskInit;
    Reg::RegTensor<IDX_T> regIdx;
    Reg::RegTensor<SCORE_T> regZero;
    Reg::Duplicate(regZero, (SCORE_T)0, maskAll);
    // Arange only supports signed index tensors; later casts reuse the same lane bits for SCORE_T comparisons.
    Reg::Arange(regIdx, static_cast<IDX_T>(0));
    Reg::Compares<SCORE_T, CMPMODE::LT>(maskInit, (Reg::RegTensor<SCORE_T> &)regIdx, (SCORE_T)initialBlocks, maskAll);

    for (uint32_t rowIdx = 0; rowIdx < rowNum; rowIdx++) {
        __ubuf__ SCORE_T *rowBuf = mrgValueBuf + rowIdx * rowStride + topkSelectNumAlign;
        Reg::StoreAlign<SCORE_T, Reg::StoreDist::DIST_NORM>(rowBuf, regZero, maskInit);
    }
}
...
template <typename SCORE_T>
__aicore__ inline void PadMrgFourRowsNotFirstS2RangeVF(const LocalTensor<SCORE_T> &mrgValueLocal,
                                                       const SICommon::RowIdx4 &rowIdx4,
                                                       const SICommon::TopkNum4 &topkNum4,
                                                       const SICommon::S2ValidLen4 &s2ValidLen4,
                                                   ...
                                                   const uint32_t windowBase)
{
    ...
    Reg::RegTensor<int32_t> regIdx;
    // regOut默认填-1，再依次用init、topK和window三个区段覆盖有效位置。
    Reg::RegTensor<uint32_t> regOut;
    // topK输出位置减去initialBlocks后，得到topkBuf中的相对下标。
    Reg::RegTensor<uint32_t> regTopkIdx;
    Reg::RegTensor<uint32_t> regTopkValue;
    // window区段直接根据输出位置生成连续索引。
    Reg::RegTensor<uint32_t> regWindowValue;
    Reg::RegTensor<uint32_t> regInitialBlocks;
    Reg::RegTensor<uint32_t> regTopkEnd;
    auto &regIdxU32 = (Reg::RegTensor<uint32_t> &)regIdx;
    ...
    Reg::Duplicate(regInitialBlocks, initialBlocks, maskAll);
    Reg::Duplicate(regTopkEnd, topkEnd, maskAll);
    ...
    for (uint32_t loopIdx = 0U; loopIdx < vfLoop; loopIdx++) {
        ...
        Reg::Arange(regIdx, static_cast<int32_t>(blockBase));
        Reg::Duplicate(regOut, static_cast<uint32_t>(-1), maskAll);

        // init区段直接输出位置本身，即0、1、2、...、initialBlocks-1。
        Reg::MaskReg maskInit;
        Reg::Compares<uint32_t, CMPMODE::LT>(maskInit, regIdxU32, initialBlocks, maskAll);
        Reg::Select(regOut, regIdxU32, regOut, maskInit);

        // topK区段通过Gather读取已经计算完成的global index。
        Reg::MaskReg maskTopkStart;
        Reg::MaskReg maskTopkEnd;
        Reg::MaskReg maskTopk;
        Reg::Compares<uint32_t, CMPMODE::GE>(maskTopkStart, regIdxU32, initialBlocks, maskAll);
        Reg::Compares<uint32_t, CMPMODE::LT>(maskTopkEnd, regIdxU32, topkEnd, maskAll);
        Reg::And(maskTopk, maskTopkStart, maskTopkEnd, maskAll);
        Reg::Sub(regTopkIdx, regIdxU32, regInitialBlocks, maskTopk);
        Reg::Gather(regTopkValue, topkBuf, regTopkIdx, maskTopk);
        Reg::Select(regOut, regTopkValue, regOut, maskTopk);

        // window区段生成windowBase开始的连续索引，并拼在topK区段之后。
        Reg::MaskReg maskWindowStart;
        Reg::MaskReg maskWindowEnd;
        Reg::MaskReg maskWindow;
        Reg::Compares<uint32_t, CMPMODE::GE>(maskWindowStart, regIdxU32, topkEnd, maskAll);
        Reg::Compares<uint32_t, CMPMODE::LT>(maskWindowEnd, regIdxU32, windowEnd, maskAll);
        Reg::And(maskWindow, maskWindowStart, maskWindowEnd, maskAll);
        Reg::Sub(regWindowValue, regIdxU32, regTopkEnd, maskWindow);
        Reg::Adds(regWindowValue, regWindowValue, windowBase, maskWindow);
        Reg::Select(regOut, regWindowValue, regOut, maskWindow);

        // 最后一轮仅写packLen以内的lane，避免覆盖当前行固定输出区间之外的数据。
        Reg::MaskReg maskPack;
        Reg::Compares<uint32_t, CMPMODE::LT>(maskPack, regIdxU32, packLen, maskAll);
        Reg::StoreAlign<uint32_t, Reg::StoreDist::DIST_NORM>(outBuf + blockBase, regOut, maskPack);
    }
}
