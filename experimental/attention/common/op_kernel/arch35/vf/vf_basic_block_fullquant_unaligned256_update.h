...
__simd_vf__ void ProcessVec1UpdateGeneralImpl256NZVF(__ubuf__ T2 *expUb, __ubuf__ T *srcUb, __ubuf__ T *inMaxUb,
                                                     ...
                                                     const T minValue, const float pScale)
{
    ...
    StoreAlign<half, Reg::StoreDist::DIST_NORM_B16>(((__ubuf__ half *&)tmpMaxUb2), vreg_max_new, preg_real_m);
    LocalMemBar<MemType::VEC_STORE, MemType::VEC_LOAD>();
    ...
    for (uint16_t i = 0; i < (m / 16); ++i) {
        ...
        for (uint16_t j = 0; j < n / 32; ++j) {
            LoadAlign<half, Reg::LoadDist::DIST_NORM>(vreg_input_x_1, srcUb + i * 16 * 16 + j * m * 32);
            LoadAlign<half, Reg::LoadDist::DIST_NORM>(vreg_input_x_2, srcUb + i * 16 * 16 + j * m * 32 + 8 * 16);
            LoadAlign<half, Reg::LoadDist::DIST_NORM>(vreg_input_x_unroll_1, srcUb + i * 16 * 16 + j * m * 32 + m * 16);
            LoadAlign<half, Reg::LoadDist::DIST_NORM>(vreg_input_x_unroll_2,
                                                      srcUb + i * 16 * 16 + j * m * 32 + m * 16 + 8 * 16);
    ...
    const float dScaleQK, const T minValue, const float pScale)
{
    ...
    ProcessVec1UpdateGeneralImpl256NZVF<T, T2, OUTPUT_T, s1BaseSize, s2BaseSize, hasAtten, pseMode, hasDrop, isMlaSgd>(
        expUb, srcUb, inMaxUb, tmpExpSumUb, tmpMaxUb, tmpMaxUb2, indexesUb, m, originN, m, minValue, pScale);
}
} // namespace FaVectorApi
