...
__simd_vf__ void ProcessVec1NoUpdateGeneralImpl256NZVF(__ubuf__ T2 *expUb, __ubuf__ T *srcUb, __ubuf__ T *maxUb,
                                                       __ubuf__ T *maxUb2, __ubuf__ float *expSumUb,
                                                       __ubuf__ uint8_t *indexesUb, const uint16_t m, const uint32_t n,
                                                       const T minValue, const float pScale)
{
    ...
    for (uint16_t i = 0; i < (m / 16); ++i) {                           // 一次循环处理[32, 16, 16]
        ...
        StoreUnAlign<half, Reg::PostLiteral::POST_MODE_UPDATE>(((__ubuf__ half *&)maxUb), vreg_max_tmp, ureg_max, 8);
    ...
    for (uint16_t i = 0; i < (m / 16); ++i) {
        LoadAlign<half, Reg::LoadDist::DIST_E2B_B16>(vreg_max, maxUb2 + i * 16);
        ...
        for (uint16_t j = 0; j < n / 32; ++j) {
            ...
            LoadAlign<half, Reg::LoadDist::DIST_NORM>(vreg_input_x_2, srcUb + i * 16 * 16 + j * 64 * 32 + 8 * 16);
            ...
            LoadAlign<half, Reg::LoadDist::DIST_NORM>(vreg_input_x_unroll_2,
                                                      srcUb + i * 16 * 16 + j * 64 * 32 + 64 * 16 + 8 * 16);
    ...
    const float dScaleQK, const T minValue, const float pScale)
{
    ...
    ProcessVec1NoUpdateGeneralImpl256NZVF<T, T2, OUTPUT_T, s1BaseSize, s2BaseSize, hasAtten, pseMode, hasDrop,
                                          isMlaSgd>(expUb, srcUb, maxUb, maxUb2, expSumUb, indexesUb, m, originN,
                                                    minValue, pScale);
}
} // namespace FaVectorApi
