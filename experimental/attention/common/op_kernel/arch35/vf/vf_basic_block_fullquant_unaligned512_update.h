...
__simd_vf__ void ProcessVec1UpdateGeneralImpl512NZVF(__ubuf__ T2 *expUb1, __ubuf__ T2 *expUb2, __ubuf__ T *srcUb1,
                                                     __ubuf__ T *srcUb2, __ubuf__ T *inMaxUb,
                                                     __ubuf__ float *tmpExpSumUb, __ubuf__ T *tmpMaxUb,
                                                     __ubuf__ T *tmpMaxUb2, __ubuf__ uint8_t *indexesUb,
                                                     const uint16_t m, const uint32_t n, uint32_t realM,
                                                     const T minValue, const float pScale)
{
    ...
    StoreAlign<half, Reg::StoreDist::DIST_NORM_B16>(((__ubuf__ half *&)tmpMaxUb2), vreg_max_new,
                                                    preg_real_m); // 64个max，128B
    LocalMemBar<MemType::VEC_STORE, MemType::VEC_LOAD>();

    for (uint16_t i = 0; i < (m / 16); ++i) {
        LoadAlign<half, Reg::LoadDist::DIST_E2B_B16>(vreg_max, tmpMaxUb2 + i * 16); // 读入8行的max并广播
        ...
        for (uint16_t j = 0; j < n / 32; ++j) {
            ...
            LoadAlign<half, Reg::LoadDist::DIST_NORM>(
                ...
                srcUb1 + i * 16 * 16 + j * m * 32 + m * 16); // 第二个分形的前8行（第二个[64,16]）

            LoadAlign<half, Reg::LoadDist::DIST_NORM>(vreg_input_x_2,
                                                      srcUb1 + i * 16 * 16 + j * m * 32 + 8 * 16); // 第一个分形的后8行
    ...
    StoreUnAlignPost<float, Reg::PostLiteral::POST_MODE_UPDATE>(((__ubuf__ float *&)tmpExpSumUb), ureg_exp_sum, 0);
}
    ...
    const float dScaleQK, const T minValue, const float pScale)
{
    ...
    ProcessVec1UpdateGeneralImpl512NZVF<half, T2, OUTPUT_T, s1BaseSize, s2BaseSize, hasAtten, pseMode, hasDrop,
                                        isMlaSgd>(expUb1, expUb2, srcUb1, srcUb2, inMaxUb, tmpExpSumUb, tmpMaxUb,
                                                  tmpMaxUb2, indexesUb, m, n, m, minValue, pScale);
}
} // namespace FaVectorApi
