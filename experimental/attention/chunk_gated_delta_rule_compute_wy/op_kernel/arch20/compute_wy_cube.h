...
public:
        ...
        for (uint32_t k0 = 0; k0 < kDim; k0 += WY_CUBE_CHUNK) {
            ...
            if (!(k0 == 0 && aLda == kCur)) {
                Muls(halfScratch, aUb[k0], static_cast<half>(1), static_cast<uint64_t>(kCur), WY_CUBE_CHUNK,
                     {1, 1, static_cast<uint8_t>(kCur * sizeof(half) / 32),
                      static_cast<uint8_t>(aLda * sizeof(half) / 32)});
                aFeed = halfScratch;
            }
            if (!(k0 == 0 && bLda == kCur)) {
                Muls(halfScratch[WY_CUBE_CHUNK * WY_CUBE_CHUNK], bUb[k0], static_cast<half>(1),
                     static_cast<uint64_t>(kCur), WY_CUBE_CHUNK,
                     {1, 1, static_cast<uint8_t>(kCur * sizeof(half) / 32),
    ...
    __aicore__ inline void GemmApplyReplace(LocalTensor<float> rUb, LocalTensor<half> tUbHalf,
                                            LocalTensor<half> bScratch, LocalTensor<float> floatScratch, uint32_t nDim,
