...
__aicore__ inline uint16_t BytesToBlocks(uint32_t bytes)
{
    return static_cast<uint16_t>(AlignUp(bytes, BLOCK_BYTES) / BLOCK_BYTES);
}
...
public:
                                ...
                                const ChunkGatedDeltaRuleComputeWyTilingData *tiling, TPipe *pipe)
    {
        ...
        const uint32_t qHalfElems =
            FIXED_CHUNK_SIZE * (maxAlign < 2 * FIXED_CHUNK_SIZE ? 2 * FIXED_CHUNK_SIZE : maxAlign);
        ...
        // those tasks took the doubling path).
        {
            ...
            if (halfBytes < cScratchBytes)
                halfBytes = cScratchBytes;
...
private:
                                         ...
                                         uint32_t alignDim) const
    {
        ...
        DataCopyParams copyParams{FIXED_CHUNK_SIZE, BytesToBlocks(rowBytes),
                                  BytesToBlocks(srcRowStrideBytes - rowBytes), dstStrideBlocks};
                                               ...
                                               uint32_t tokenStart, uint32_t headIdx, uint32_t numHeads)
    {
        ...
        if (span <= scratchElems && numHeads == vNumHead_) {
            ...
            SyncEvent<HardEvent::MTE2_V>(HardEvent::MTE2_V);
            Gather(dstLocal, scratch, gOffBuf_.Get<uint32_t>(), headIdx * static_cast<uint32_t>(sizeof(float)),
                   FIXED_CHUNK_SIZE);
                                         ...
                                         uint32_t b, uint32_t tokenStart, uint32_t vHeadIdx)
    {
        ...
        if (span <= scratchElems) {
            ...
            Gather(betaHalf, scratch, betaOffBuf_.Get<uint32_t>(), vHeadIdx * static_cast<uint32_t>(sizeof(half)),
                   FIXED_CHUNK_SIZE);
            PipeBarrier<PIPE_V>();
    ...
    __aicore__ inline void BuildCumulativeG(uint32_t b, uint32_t vHeadIdx, uint32_t tokenStart,
                                            LocalTensor<float> gLocal, LocalTensor<float> expGLocal,
                                            LocalTensor<float> gRaw, LocalTensor<float> loadScratch,
                                            uint32_t loadScratchElems)
    {
                                                 ...
                                                 uint32_t srcLda) const
    {
        ...
        const BinaryRepeatParams rp{
            1, 1, 0, static_cast<uint8_t>(dstLda / FLOAT_PER_BLOCK), static_cast<uint8_t>(srcLda / FLOAT_PER_BLOCK), 1};
        uint32_t col = 0;
                                                ...
                                                uint32_t srcLda) const
    {
        ...
        PipeBarrier<PIPE_V>();
        const BinaryRepeatParams rp{
            1, 1, 0, static_cast<uint8_t>(dstLda / HALF_PER_BLOCK), static_cast<uint8_t>(srcLda / HALF_PER_BLOCK), 1};
        uint32_t col = 0;
    ...
    __aicore__ inline bool NeedsFp32ForwardSubstitution(const LocalTensor<float> a, LocalTensor<float> absScratch,
                                                        LocalTensor<float> rowSums, LocalTensor<float> reduceScratch)
    {
        ...
        for (uint32_t w = FIXED_CHUNK_SIZE / 2; w >= FLOAT_PER_BLOCK; w >>= 1) {
            const BinaryRepeatParams rp{
                1, 1, 1, static_cast<uint8_t>(rowBlk), static_cast<uint8_t>(rowBlk), static_cast<uint8_t>(rowBlk)};
                                                   ...
                                                   LocalTensor<float> cScratch)
    {
        ...
        for (uint32_t b0 = 0; b0 < FIXED_CHUNK_SIZE; b0 += BLK) {
            if (b0 > 0) {
                ...
                // R_b += C
                const BinaryRepeatParams arp{1,
                                             1,
                                             1,
                                                    ...
                                                    uint32_t cols, uint32_t srcLda)
    {
        if (srcLda == cols) {
            ...
            Cast(dst, src, RoundMode::CAST_NONE, static_cast<uint64_t>(cols > 64 ? 64 : cols),
                 static_cast<uint8_t>(rows),
                 {1, 1, static_cast<uint8_t>(cols * sizeof(half) / 32),
                  static_cast<uint8_t>(srcLda * sizeof(float) / 32)});
            if (cols > 64) {
                Cast(dst[64], src[64], RoundMode::CAST_NONE, static_cast<uint64_t>(cols - 64),
                     static_cast<uint8_t>(rows),
                     {1, 1, static_cast<uint8_t>(cols * sizeof(half) / 32),
                      static_cast<uint8_t>(srcLda * sizeof(float) / 32)});
            }
    ...
    __aicore__ inline void BuildTMat(LocalTensor<float> attnLocal, LocalTensor<float> tOut, LocalTensor<float> cScratch,
                                     LocalTensor<half> pHalf, LocalTensor<half> tHalf, LocalTensor<float> cNz)
    {
        (void)cNz;
    ...
    WyMicroMm microMm_;
    TBuf<TPosition::VECCALC> halfBuf_, qHalfBuf_, rhsBuf_, attnBuf_, gBuf_, expGBuf_, tmpBuf_, rowBuf_, negABuf_,
        lamOffBuf_, gOffBuf_, betaOffBuf_, lane0OffBuf_, betaHalfBuf_, storeBuf_, mmLocalWsBuf_;
};
