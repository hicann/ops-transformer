                                           ...
                                           const T minValue, uint32_t pltOriginalN, uint32_t pltSrcN)
{
    ...
    AscendC::Reg::RegTensor<bfloat16_t> vreg_dst_even_bf16;
    AscendC::Reg::RegTensor<bfloat16_t> vreg_dst_odd_bf16;
    ...
    AscendC::Reg::MaskReg preg_src_n = AscendC::Reg::UpdateMask<T>(pltSrcN);
    ...
    for (uint16_t i = 0; i < m; ++i) {
        ...
        AscendC::Reg::Muls(vreg_input_x, vreg_input_x, scale, preg_ori_src_n);
    ...
    AscendC::Reg::LoadAlign(vreg_cur_max, tmpMaxUb2);
    AscendC::Reg::Max(vreg_max_new, vreg_cur_max, vreg_in_max, preg_all); // 计算新、旧的最大值
    ...
    for (uint16_t i = 0; i < m; ++i) {
        ...
        AscendC::Reg::Reduce<Reg::ReduceType::SUM, float, float, Reg::MaskMergeMode::ZEROING>(vreg_exp_sum, vreg_exp,
                                                                                              preg_ori_src_n);
        ...
        if constexpr (IsSameType<T2, hifloat8_t>::value) {
            AscendC::Reg::Muls(vreg_exp, vreg_exp, hifp8MaxValue, preg_ori_src_n);
        }
        if constexpr (IsSameType<T2, bfloat16_t>::value) {
            AscendC::Reg::Cast<T2, T, castTraitZero>(vreg_exp_bf16, vreg_exp, preg_all_b16);
            AscendC::Reg::DeInterleave(vreg_dst_even_bf16, vreg_dst_odd_bf16, vreg_exp_bf16, vreg_exp_bf16);
            ...
            AscendC::Reg::RegTensor<hifloat8_t> vreg_exp_merge_hifp8;
            uint32_t maskLen = 128;
            AscendC::Reg::MaskReg preg_all_b8_128 = UpdateMask<T2>(maskLen);
            AscendC::Reg::Cast<T2, T, castTraitZero>(vreg_exp_hifp8, vreg_exp, preg_all);
            AscendC::Reg::LoadAlign(vreg_exp_merge_hifp8_indexes, indexesUb);
            AscendC::Reg::Gather(vreg_exp_merge_hifp8, vreg_exp_hifp8, vreg_exp_merge_hifp8_indexes);
            AscendC::Reg::StoreAlign<T2, Reg::DataCopyMode::DATA_BLOCK_COPY, Reg::PostLiteral::POST_MODE_UPDATE>(
