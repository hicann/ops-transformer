...
ge::graphStatus BlockSparseAttentionTiling::GetNpuInfo(gert::TilingContext *bsaContext)
{
...
ge::graphStatus BlockSparseAttentionTiling::CheckNDFormat(gert::TilingContext *bsaContext)
{
    auto queryFormat =
...
ge::graphStatus BlockSparseAttentionTiling::ValidateTNDSeqlenSum(gert::TilingContext *bsaContext)
{
    // 只在TND格式时进行校验
...
ge::graphStatus BlockSparseAttentionTiling::GetInputLayout(gert::TilingContext *bsaContext)
{
...
ge::graphStatus BlockSparseAttentionTiling::CheckQKVDtype(gert::TilingContext *bsaContext)
{
...
ge::graphStatus BlockSparseAttentionTiling::CheckQKVDimVal(gert::TilingContext *bsaContext,
                                                           uint32_t kHeads, uint32_t vHeads,
...
ge::graphStatus BlockSparseAttentionTiling::ParseQKVInTND(gert::TilingContext *bsaContext)
{
...
ge::graphStatus BlockSparseAttentionTiling::ParseQKVInBNSD(gert::TilingContext *bsaContext)
{
    const auto *queryShape = bsaContext->GetInputShape(QUERY_INDEX);
    const auto *keyShape = bsaContext->GetInputShape(KEY_INDEX);
    const auto *valueShape = bsaContext->GetInputShape(VALUE_INDEX);
...
ge::graphStatus BlockSparseAttentionTiling::ParseQKVInBSND(gert::TilingContext *bsaContext)
{
    const auto *queryShape = bsaContext->GetInputShape(QUERY_INDEX);
    const auto *keyShape = bsaContext->GetInputShape(KEY_INDEX);
    const auto *valueShape = bsaContext->GetInputShape(VALUE_INDEX);
...
ge::graphStatus BlockSparseAttentionTiling::CheckAttentionOutDtype(gert::TilingContext *bsaContext)
{
...
ge::graphStatus BlockSparseAttentionTiling::ParseRequiredTensors(gert::TilingContext *bsaContext)
{
...
ge::graphStatus BlockSparseAttentionTiling::ParseSeqlensInTND(gert::TilingContext *bsaContext)
{
...
ge::graphStatus BlockSparseAttentionTiling::ParseSeqlensInNonTND(gert::TilingContext *bsaContext)
{
    const auto *actualSeqLengths = bsaContext->GetOptionalInputTensor(ACTUAL_SEQ_LENGTHS_INDEX);
    const auto *actualSeqLengthsKv = bsaContext->GetOptionalInputTensor(ACTUAL_SEQ_LENGTHS_KV_INDEX);
    if (actualSeqLengths != nullptr && actualSeqLengthsKv != nullptr) {
...
ge::graphStatus BlockSparseAttentionTiling::ParseSeqlens(gert::TilingContext *bsaContext)
{
    ge::graphStatus ret = ge::GRAPH_SUCCESS;
    if (qInputLayout_ == BSAQInputLayout::TND_Q) {
...
ge::graphStatus BlockSparseAttentionTiling::CheckSparsePattern(gert::TilingContext *bsaContext,
...
ge::graphStatus BlockSparseAttentionTiling::ParseSparsePattern(gert::TilingContext *bsaContext)
{
...
ge::graphStatus BlockSparseAttentionTiling::ParseAttenMask(gert::TilingContext *bsaContext)
{
    const auto *attenMaskTensor = bsaContext->GetOptionalInputTensor(ATTEN_MASK_INDEX);
    if (attenMaskTensor != nullptr) {
...
ge::graphStatus BlockSparseAttentionTiling::ParseBlockTable(gert::TilingContext *bsaContext)
{
    const auto *blockTableTensor = bsaContext->GetOptionalInputTensor(BLOCK_TABLE_INDEX);
    if (blockTableTensor != nullptr) {
...
ge::graphStatus BlockSparseAttentionTiling::ValidateGenericDequantScale(gert::TilingContext *bsaContext,
...
ge::graphStatus BlockSparseAttentionTiling::ValidateQDequantScale(gert::TilingContext *bsaContext)
{
    return ValidateGenericDequantScale(bsaContext, Q_DEQUANT_SCALE_INDEX);
}

ge::graphStatus BlockSparseAttentionTiling::ValidateKDequantScale(gert::TilingContext *bsaContext)
{
    return ValidateGenericDequantScale(bsaContext, K_DEQUANT_SCALE_INDEX);
}

ge::graphStatus BlockSparseAttentionTiling::ValidateVDequantScale(gert::TilingContext *bsaContext)
{
    return ValidateGenericDequantScale(bsaContext, V_DEQUANT_SCALE_INDEX);
}

ge::graphStatus BlockSparseAttentionTiling::ParseOptionalTensors(gert::TilingContext *bsaContext)
{
...
ge::graphStatus BlockSparseAttentionTiling::ParseAttrs(gert::TilingContext *bsaContext)
{
    auto attrs = bsaContext->GetAttrs();
    OP_CHECK_NULL_WITH_CONTEXT(bsaContext, attrs);
...
void BlockSparseAttentionTiling::CalculateBatchTaskSplit(int64_t qSeqlen, uint32_t groupSize, uint32_t &curTaskNum,
...
uint32_t BlockSparseAttentionTiling::GetCurQSTileNum950(int64_t curQSeqlen)
{
...
void BlockSparseAttentionTiling::CalcBaseTileTilingParams950()
{
...
void BlockSparseAttentionTiling::CalcSplitCoreTilingParams950()
{
...
ge::graphStatus BlockSparseAttentionTiling::CalculateTaskSplit(gert::TilingContext *bsaContext)
{
    // 计算总的Q块数量和最大KV块数量
    totalQBlocks_ = 0;
...
void BlockSparseAttentionTiling::CalcWorkspaceTilingParams950(gert::TilingContext *bsaContext)
{
    selectIdxSize_ =
...
ge::graphStatus BlockSparseAttentionTiling::CalculateWorkSpace(gert::TilingContext *bsaContext)
{
    if (blockDim_ == 0) {
...
void BlockSparseAttentionTiling::CalcMatmulPhaseL1TileInfo950()
{
...
ge::graphStatus BlockSparseAttentionTiling::FillTilingData(gert::TilingContext *bsaContext)
{
...
uint64_t BlockSparseAttentionTiling::GenerateTilingKey(gert::TilingContext *bsaContext)
{
    /**
     * 64位整数，使用十进制位域表示：
...
ge::graphStatus BlockSparseAttentionTiling::GetBsaTiling(gert::TilingContext *bsaContext,
...
ge::graphStatus BlockSparseAttentionTiling::BsaSetTilingData(gert::TilingContext *context,
                                                             BlockSparseAttentionTilingData &tilingData)
{
...
ASCENDC_EXTERN_C ge::graphStatus TilingBlockSparseAttention(gert::TilingContext *context)
{
    OP_CHECK_IF(context == nullptr, OPS_REPORT_VECTOR_INNER_ERR("BlockSparseAttention", "Context is nullptr."),
                return ge::GRAPH_FAILED);
    BlockSparseAttentionTilingData tilingData;
    BlockSparseAttentionTiling bsaTiling;
