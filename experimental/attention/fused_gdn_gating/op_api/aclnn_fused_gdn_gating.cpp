...
static const std::initializer_list<op::DataType> AB_TYPE_SUPPORT_LIST = {op::DataType::DT_BF16,
...
static inline bool CheckDtype(const FusedGdnGatingParams &params)
{
    ...
    OP_CHECK(params.a->GetDataType() == params.b->GetDataType(),
             OP_LOGE(ACLNN_ERR_PARAM_INVALID, "a and b must have the same dtype."), return false);
    OP_CHECK(params.aLog->GetDataType() == params.dtBias->GetDataType(),
             OP_LOGE(ACLNN_ERR_PARAM_INVALID, "aLog and dtBias must have the same dtype."), return false);
    OP_CHECK(params.betaOutput->GetDataType() == params.b->GetDataType(),
             OP_LOGE(ACLNN_ERR_PARAM_INVALID, "betaOutput and b must have the same dtype."), return false);
    return true;
}
...
aclnnStatus aclnnFusedGdnGatingGetWorkspaceSize(const aclTensor *aLog, const aclTensor *a, const aclTensor *b,
                                                const aclTensor *dtBias, float beta, float threshold, aclTensor *g,
                                                aclTensor *betaOutput, uint64_t *workspaceSize,
                                                aclOpExecutor **executor)
{
    L2_DFX_PHASE_1(aclnnFusedGdnGating, DFX_IN(aLog, a, b, dtBias, beta, threshold), DFX_OUT(g, betaOutput));
    ...
    auto result =
        l0op::FusedGdnGating(aLogContig, aContig, bContig, dtBiasContig, beta, threshold, uniqueExecutor.get());
    CHECK_RET(result.g != nullptr && result.beta_output != nullptr, ACLNN_ERR_INNER_NULLPTR);
...
aclnnStatus aclnnFusedGdnGating(void *workspace, uint64_t workspaceSize, aclOpExecutor *executor, aclrtStream stream)
{
