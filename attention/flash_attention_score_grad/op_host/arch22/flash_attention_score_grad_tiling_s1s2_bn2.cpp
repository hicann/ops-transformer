...
bool FlashAttentionScoreGradTilingS1s2Bn2::IsCapable()
{
    ...
    OP_CHECK_IF(context_->GetInputShape(VALUE) == nullptr,
                OP_LOGE_WITH_INVALID_INPUT("FlashAttentionScoreGrad", "value"), return ge::GRAPH_FAILED);
    OP_CHECK_IF(context_->GetInputShape(KEY) == nullptr, OP_LOGE_WITH_INVALID_INPUT("FlashAttentionScoreGrad", "keyIn"),
...
uint64_t FlashAttentionScoreGradTilingS1s2Bn2::GetTilingKey() const
{
    ...
    tilingKey = GET_TPL_TILING_KEY(
        static_cast<uint8_t>(AxisEnum::S2), static_cast<uint8_t>(AxisEnum::S1), static_cast<uint8_t>(AxisEnum::N2), 0,
        static_cast<uint8_t>(inDtype), static_cast<uint8_t>(layout), static_cast<uint8_t>(SparseEnum::ALL),
        static_cast<uint8_t>(mmConfig), static_cast<uint8_t>(mmOutFormat), static_cast<uint8_t>(mm345NZOut),
        static_cast<uint8_t>(tmpData_.drop_out_cfg), static_cast<uint8_t>(tmpData_.pse_cfg),
...
ge::graphStatus FlashAttentionScoreGradTilingS1s2Bn2::GetPlatformInfo()
{
    ...
    OP_CHECK_IF((aicoreParams_.numBlocks == 0) || (aicoreParams_.aicNum == 0),
                OP_LOGE(context_,
                        ...
                        "is %lu, num of aicNum is %lu.",
                        aicoreParams_.numBlocks, aicoreParams_.aicNum),
                return ge::GRAPH_FAILED);

    OP_CHECK_IF(
        aicoreParams_.ubSize <= 0 || l2CacheSize <= 0,
        OP_LOGE(
            context_,
            "The op [FlashAttentionScoreGrad] received bad params, the reason is: ubSize or l2CacheSize is invalid."),
        return ge::GRAPH_FAILED);
                                                                       ...
                                                                       const uint32_t maskShapeDims)
{
    if (maskShapeDims == DIM_2) {
        ...
        if ((dim0 == td_->opInfo.get_B()) && (dim1 == td_->opInfo.get_N2() * td_->opInfo.get_G())) {
            ...
            std::string shapeMsg = Ops::Base::ToString(storageShape);
        ...
        OP_LOGE_FOR_INVALID_SHAPEDIM_WITH_REASON("FlashAttentionScoreGrad", "attenMaskOptional",
                                                 std::to_string(maskShapeDims).c_str(),
        ...
        if (attenMaskDesc != nullptr) {
            ...
            if (attenMaskDtype == tmpData_.queryType) {
                ...
                OP_LOGE_FOR_INVALID_DTYPE_WITH_REASON(
                    ...
                    ge::TypeUtils::DataTypeToSerialString(attenMaskDtype).c_str(),
                    "The dtype of attenMaskOptional must match query or be BOOL or UINT8");
                return ge::GRAPH_FAILED;
            }
        }
    }
...
ge::graphStatus FlashAttentionScoreGradTilingS1s2Bn2::ProcessNormalPse()
{
    ...
    if (pseShapeDims != DIM_4) {
        OP_LOGE_FOR_INVALID_SHAPEDIM("FlashAttentionScoreGrad", "pseShiftOptional",
                                     std::to_string(pseShapeDims).c_str(), "4");
        return ge::GRAPH_FAILED;
    }
    auto dim0 = storageShape.GetDim(DIM_0);
    auto dim1 = storageShape.GetDim(DIM_1);
    ...
    if (isUnpad && isNHS && (dim0 == 1) && alibiCondi2) {
        ...
        std::string reasonMsg = "The shape of pseShiftOptional is unsupported when preTokens is " +
                                std::to_string(td_->opInfo.get_preTokens()) + " and nextTokens is " +
                                std::to_string(td_->opInfo.get_nextTokens());
    ...
    */
    ...
    if (pseShape == nullptr || pseShape->GetStorageShape().GetDimNum() == 0) {
        if (tmpData_.pseType != PSE_OUTER_ADD_MUL_TYPE) {
             ...
             * FA反向未兼容，因此统一拦截异常输入。
             */
            std::string valuesMsg = "{empty, " + std::to_string(tmpData_.pseType) + "}";
    ...
    if (pse != nullptr && pseShape != nullptr && pseShape->GetStorageShape().GetDimNum() != 0) {
        ...
        if (tmpData_.pseType == PSE_OUTER_MUL_ADD_TYPE || tmpData_.pseType == PSE_OUTER_ADD_MUL_TYPE) {
            OP_CHECK_IF(
                pse->GetDataType() != context_->GetInputDesc(QUERY)->GetDataType(),
        ...
        if (tmpData_.pseType == PSE_INNER_MUL_ADD_TYPE || tmpData_.pseType == PSE_INNER_MUL_ADD_SQRT_TYPE) {
            if (pseShapeDims == PSE_DIM_NUM_1) {
                ...
                OP_CHECK_IF(dim0 != tmpData_.n2 * tmpData_.g,
                            OP_LOGE_FOR_INVALID_SHAPE("FlashAttentionScoreGrad", "pseShiftOptional",
                                                      ...
                                                      ("[" + std::to_string(tmpData_.n2 * tmpData_.g) + "]").c_str()),
                            return ge::GRAPH_FAILED);
                ...
                OP_CHECK_IF(dim0 != tmpData_.b || dim1 != tmpData_.n2 * tmpData_.g,
                            OP_LOGE_FOR_INVALID_SHAPE("FlashAttentionScoreGrad", "pseShiftOptional",
                                                      Ops::Base::ToString(pseShape->GetStorageShape()).c_str(),
                                                      ("[" + std::to_string(tmpData_.b) + ", " +
                                                       std::to_string(tmpData_.n2 * tmpData_.g) + "]")
                                                          .c_str()),
                            return ge::GRAPH_FAILED);
                td_->opInfo.set_pseShapeType(PSE_B_N2_G_SLOPE);
            } else {
                OP_LOGE_FOR_INVALID_SHAPEDIM_WITH_REASON(
                    "FlashAttentionScoreGrad", "pseShiftOptional", std::to_string(pseShapeDims).c_str(),
            ...
            if (isUnpadBN1S) {
                ...
                OP_LOGE_FOR_INVALID_SHAPE_WITH_REASON(
                    "FlashAttentionScoreGrad", "pseShiftOptional", Ops::Base::ToString(storageShape).c_str(),
...
ge::graphStatus FlashAttentionScoreGradTilingS1s2Bn2::SetSparseParams()
{
    ...
    if (status == false) {
        ...
        OP_LOGE_FOR_INVALID_ARGUMENT_WITH_REASON(
            "FlashAttentionScoreGrad", "sparseMode, attenMaskOptional, prefixOptional, preTokens, nextTokens",
            "The sparse configuration must be supported when sparseMode is prefix compress");
        return ge::GRAPH_FAILED;
    }
...
ge::graphStatus FlashAttentionScoreGradTilingS1s2Bn2::CheckOutOfTokens(const int64_t s1, const int64_t s2)
{
    ...
    if (-td_->opInfo.get_preTokens() > negPretMax || -td_->opInfo.get_nextTokens() > negNexttMax ||
        ...
        std::string valuesMsg = "{" + std::to_string(s1) + ", " + std::to_string(s2) + ", " +
                                std::to_string(td_->opInfo.get_preTokens()) + ", " +
                                std::to_string(td_->opInfo.get_nextTokens()) + "}";
        OP_LOGE_FOR_INVALID_VALUES_WITH_REASON(
            "FlashAttentionScoreGrad", "S1, S2, preTokens, nextTokens", valuesMsg.c_str(),
            "The preTokens and nextTokens ranges are invalid for the input sequence lengths");
        return ge::GRAPH_FAILED;
    }
    return ge::GRAPH_SUCCESS;
}
...
ge::graphStatus FlashAttentionScoreGradTilingS1s2Bn2::GetShapeAttrsInfo()
{
    ...
    */
    ...
    OP_CHECK_IF(
        (td_->opInfo.get_keepProb() <= 0 || td_->opInfo.get_keepProb() > 1),
        OP_LOGE_WITH_INVALID_ATTR("FlashAttentionScoreGrad", "keepProb",
                                  std::to_string(td_->opInfo.get_keepProb()).c_str(), "a value in the range (0, 1]"),
        return ge::GRAPH_FAILED);
    ...
    if (attrs->GetAttrNum() > static_cast<size_t>(SPARSE_MODE)) {
        ...
        if (sparse > BAND_LEFT_UP_CASUAL) {
            OP_LOGE_WITH_INVALID_ATTR("FlashAttentionScoreGrad", "sparseMode", std::to_string(sparse).c_str(),
                                      "a value from 0 to 8");
            return ge::GRAPH_FAILED;
        }
    }
    ...
    if ((td_->opInfo.get_layout() != static_cast<uint32_t>(InputLayout::TND)) &&
        ...
        std::string valuesMsg = "{" + std::string(inputLayout) + ", " + std::to_string(sparseMode) + "}";
    ...
    if (attrs->GetAttrNum() > static_cast<size_t>(PSETYPE)) {
        ...
        OP_CHECK_IF(
            (tmpData_.pseType < 0 || tmpData_.pseType >= PSE_INVALID_TYPE),
            OP_LOGE_WITH_INVALID_ATTR("FlashAttentionScoreGrad", "pseType", std::to_string(tmpData_.pseType).c_str(),
...
ge::graphStatus FlashAttentionScoreGradTilingS1s2Bn2::CheckAttenMaskShape()
{
    ...
    if (attenMaskCompressMode == NO_COMPRESS_MODE) {
        ...
        if (invalid) {
            std::string shapeMsg = "[" + std::to_string(attenMaskS1Size) + ", " + std::to_string(attenMaskS2Size) + "]";
            std::string expectedMsg = "[" + std::to_string(tmpData_.s1) + ", " + std::to_string(tmpData_.s2) + "]";
...
ge::graphStatus FlashAttentionScoreGradTilingS1s2Bn2::DoBlockTiling()
{
    ...
    td_->opInfo.set_formerCoreNum((tmpData_.isL1CustomEnable ? sameAbBlockDim : aicoreParams_.numBlocks) -
                                  remainCoreNum);
...
ge::graphStatus FlashAttentionScoreGradTilingS1s2Bn2::DoCastTiling()
{
    ...
    OP_CHECK_IF(
        qPostBaseNum == 0,
        OP_LOGE(context_, "The op [FlashAttentionScoreGrad] received bad params, the reason is: qPostBaseNum is 0."),
        return ge::GRAPH_FAILED);
    OP_CHECK_IF(
        usedCoreNum == 0,
        OP_LOGE(context_, "The op [FlashAttentionScoreGrad] received bad params, the reason is: castUsedCoreNum is 0."),
        return ge::GRAPH_FAILED);
    ...
    OP_CHECK_IF(
        kvPostBaseNum == 0,
        OP_LOGE(context_, "The op [FlashAttentionScoreGrad] received bad params, the reason is: kvPostBaseNum is 0."),
        return ge::GRAPH_FAILED);
...
ge::graphStatus FlashAttentionScoreGradTilingS1s2Bn2::GetWorkspaceSize()
{
    ...
    OP_CHECK_IF(blockdim == 0,
                OP_LOGE(context_,
                        "The op [FlashAttentionScoreGrad] received bad params, the reason is: blockdim is 0, aicNum is "
                        "%lu, aivNum is %lu.",
    ...
    workspaceOffsets += (td_->opInfo.get_mm4InputWorkspaceLen() + td_->opInfo.get_mm3InputWorkspaceLen()) *
                        td_->opInfo.get_usedCoreNum();
...
{
    OP_CHECK_IF(context_->GetInputShape(QUERY) == nullptr,
    ...
    OP_CHECK_IF(!ret,
                OP_LOGE_FOR_INVALID_SHAPES_WITH_REASON(
                    ...
                    ("{" + Ops::Base::ToString(qShape->GetStorageShape()) + ", " +
    ...
    OP_CHECK_IF(!ret,
                OP_LOGE_FOR_INVALID_SHAPES_WITH_REASON(
                    ...
                    ("{" + Ops::Base::ToString(kShape->GetStorageShape()) + ", " +
                     Ops::Base::ToString(vShape->GetStorageShape()) + "}")
                        .c_str(),
...
ge::graphStatus FlashAttentionScoreGradTilingS1s2Bn2::ProcessDropInfo()
{
    ...
    OP_CHECK_IF(dropMaskShapeSize == 0,
                OP_LOGE_FOR_INVALID_SHAPESIZE_WITH_REASON(
                    "FlashAttentionScoreGrad", "dropMaskOptional", std::to_string(dropMaskShapeSize).c_str(),
                    "dropMaskOptional cannot have zero elements when keepProb is less than 1"),
                return ge::GRAPH_FAILED);
...
REGISTER_TILING_TEMPLATE_WITH_ARCH(FlashAttentionScoreGrad, FlashAttentionScoreGradTilingS1s2Bn2,
                                   std::vector<int32_t>({static_cast<int32_t>(NpuArch::DAV_2201)}), 15000);
REGISTER_TILING_TEMPLATE_WITH_ARCH(FlashAttentionScoreGrad, FlashAttentionScoreGradTilingDeterministic,
                                   std::vector<int32_t>({static_cast<int32_t>(NpuArch::DAV_2201)}), 1000);
