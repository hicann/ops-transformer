...
#define SFA_OP_IMPL(templateClass, tilingdataClass, ...) \
    do { \
        ...
        op.Init(query, key, value, sparseIndices, actualSeqLengthsQuery, actualSeqLengthsKV, blocktable, queryRope, \
                keyRope, attentionOut, softmaxMax, softmaxSum, sinks, user, nullptr, tiling, &tPipe); \
        op.Process(); \
    } while (0)
#else
#define SFA_OP_IMPL(templateClass, tilingdataClass, ...) \
    do { \
        ...
        op.Init(query, key, value, sparseIndices, actualSeqLengthsQuery, actualSeqLengthsKV, blocktable, queryRope, \
                keyRope, attentionOut, softmaxMax, softmaxSum, sinks, user, tilingData, tiling, &tPipe); \
        op.Process(); \
    } while (0)
#endif
#else
...
__global__ __aicore__ void sparse_flash_attention(
    ...
    __gm__ uint8_t *queryRope, __gm__ uint8_t *keyRope, __gm__ uint8_t *sinks, __gm__ uint8_t *attentionOut,
    __gm__ uint8_t *softmaxMax, __gm__ uint8_t *softmaxSum, __gm__ uint8_t *workspace, __gm__ uint8_t *tiling)
{
    ...
    if constexpr (ORIG_DTYPE_QUERY == DT_FLOAT16 && ORIG_DTYPE_KEY == DT_FLOAT16 &&
        ...
        SFA_OP_IMPL(SparseFlashAttentionMla, SparseFlashAttentionTilingDataMla, half, half, half, FLASH_DECODE,
                    static_cast<SFA_LAYOUT>(LAYOUT_T), static_cast<SFA_LAYOUT>(KV_LAYOUT_T), TEMPLATE_MODE, HAS_ROPE);
    } else { // bf16
