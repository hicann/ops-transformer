...
def _read_cache(
    ...
    while done < n:
        ...
        if cnt > n - done:
            cnt = n - done
        chunks.append(state[bid:bid + 1, off:off + cnt, d_start:d_end].reshape(cnt, d_end - d_start))
        done += cnt
...
def _write_cache(
    ...
    end_seq: int,
    cache_mode: int,
    update_mask: Optional[torch.Tensor] = None,   # 与 state 同 shape 的 bool 张量，记录写入位置
) -> torch.Tensor:
    """
...
def _build_ape_for_flat_sequence(
    ...
    """
    ape_out = torch.zeros(total_len, ape.shape[1], dtype=torch.float32, device=ape.device)
    for b in range(B):
        sp = start_pos[b]
        if bs_combine:
        ...
        if L <= 0:
            continue
        idx = (torch.arange(sp, sp + L) % cmp_ratio).long()
...
def compressor_forward(
    ...
    if bs_combine:
        # x 形状为 (T, H)，直接做矩阵乘法
        flat_kv = x_f32 @ wkv_f32.T      # (T, coff*head_dim)
        flat_score = x_f32 @ wgate_f32.T
        T = x.shape[0]
    else:
        # x 形状为 (B, S, H)，先展平再乘
        B_in, S_in, H_in = x.shape
        flat_kv = x_f32.reshape(B_in * S_in, H_in) @ wkv_f32.T
        flat_score = x_f32.reshape(B_in * S_in, H_in) @ wgate_f32.T
        T = B_in * S_in
    ...
    ape_add = _build_ape_for_flat_sequence(T, B, start_pos, ape_f32, cmp_ratio, bs_combine, cu_seqlens)
    ...
    if not bs_combine:
        ...
        cmp_kv = torch.zeros(B, max_blocks, head_dim, dtype=torch.float32, device=x.device)
        cmp_mask = torch.zeros(B, max_blocks, head_dim, dtype=torch.bool, device=x.device)
    else:
        T_val = x.shape[0]
    ...
    if return_intermediates:
        if not bs_combine:
            kvIntermediate = torch.zeros(B, max_blocks, N_inter, head_dim, dtype=torch.float32, device=x.device)
            softmaxScoreIntermediate = torch.zeros(B, max_blocks, N_inter, head_dim, dtype=torch.float32, device=x.device)
        else:
            kvIntermediate = torch.zeros(out_rows, N_inter, head_dim, dtype=torch.float32, device=x.device)
            softmaxScoreIntermediate = torch.zeros(out_rows, N_inter, head_dim, dtype=torch.float32, device=x.device)
    ...
    for b_idx in range(B):
        ...
        while batch_seq_idx < b_seq_used:
            ...
            if save_flag:
                kv_state_out = _write_cache(kv_state_out, block_table,
                                            flat_kv[off_s:off_e, :], b_idx,
                                            s_start, s_end, cache_mode,
                                            kv_update_mask if return_update_mask else None)
                score_state_out = _write_cache(score_state_out, block_table,
                                               flat_score[off_s:off_e, :], b_idx,
                                               s_start, s_end, cache_mode,
            ...
            if compress_flag:
                ...
                sc_score = torch.full((coff, cmp_ratio, head_dim), float('-inf'), device=x.device)
                ...
                if b_start == s_start:
                    ...
                    if cfs > 0:
                        kvLocal[cur_cid, 0:cfs, :] = _read_cache(
                            kv_state_out, block_table, b_idx,
                            b_start - cfs, b_start, ds_cur, de_cur, cache_mode)
                        sc_score[cur_cid, 0:cfs, :] = _read_cache(
                            score_state_out, block_table, b_idx,
                            b_start - cfs, b_start, ds_cur, de_cur, cache_mode)

                # 从展平数组获取当前数据（kv 无 ape，score 已预加 ape）
                kvLocal[cur_cid, cfs:cmp_ratio, :] = flat_kv[off_s:off_e, ds_cur:de_cur]
                sc_score[cur_cid, cfs:cmp_ratio, :] = flat_score[off_s:off_e, ds_cur:de_cur]
                    ...
                    if b_start == s_start:
                        ...
                        if b_start >= cmp_ratio:
                            ...
                            kvLocal[prev_cid, 0:cfs_p, :] = _read_cache(
                                kv_state_out, block_table, b_idx,
                                cs2, cs2 + cfs_p, ds_pr, de_pr, cache_mode)
                        ...
                        if cfs_p > 0:
                            ...
                            kvLocal[prev_cid, 0:cfs_p, :] = _read_cache(
                                kv_state_out, block_table, b_idx,
                                cs2, b_start, ds_pr, de_pr, cache_mode)
                            sc_score[prev_cid, 0:cfs_p, :] = _read_cache(
                                score_state_out, block_table, b_idx,
                                cs2, b_start, ds_pr, de_pr, cache_mode)
                    if cfs_p < cmp_ratio:
                        ...
                        po_e = off_s
                        kvLocal[prev_cid, cfs_p:cmp_ratio, :] = flat_kv[po_s:po_e, ds_pr:de_pr]
                        sc_score[prev_cid, cfs_p:cmp_ratio, :] = flat_score[po_s:po_e, ds_pr:de_pr]
                ...
                sc_score_2d = torch.where(torch.isinf(sc_score_2d),
                                          torch.tensor(-1e9, device=x.device), sc_score_2d)
                # 按列 softmax → 权重
    ...
    if return_intermediates and return_update_mask:
        return (cmp_kv, cmp_mask, kv_state_out, score_state_out,
                softmaxScoreIntermediate, kvIntermediate,
                kv_update_mask, score_update_mask)
    if return_update_mask:
        return (cmp_kv, cmp_mask, kv_state_out, score_state_out,
                kv_update_mask, score_update_mask)
    if return_intermediates:
        return cmp_kv, cmp_mask, kv_state_out, score_state_out, softmaxScoreIntermediate, kvIntermediate
    return cmp_kv, cmp_mask, kv_state_out, score_state_out
...
class Compressor(torch.nn.Module):
    ...
    def forward(
        ...
        return compressor_forward(
            ...
            cmp_ratio=self.cmp_ratio, coff=self.coff, cache_mode=self.cache_mode,
            cu_seqlens=cu_seqlens, seqused=seqused,
        )
