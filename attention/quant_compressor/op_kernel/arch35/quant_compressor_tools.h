...
template <typename COMP>
__aicore__ inline void QuantCompressorVec1SliceIterator<COMP>::AdvanceToNextUsedBatch()
{
...
template <typename COMP>
__aicore__ inline bool QuantCompressorVec1SliceIterator<COMP>::SkipBatchGap()
{
    ...
    if (sliceInfo_.sIdx > 0) {
        uint64_t nextAlignSIdx = Align(sliceInfo_.bStartPos + sliceInfo_.sIdx, (cmpRatio)) - sliceInfo_.bStartPos;
