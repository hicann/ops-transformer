...
import torch
from batch import quant_compressor_pt_loadprocess
import pytest
import pandas as pd
from pathlib import Path
import numpy as np
import os
from quant_compressor_golden import check_result
...
# 支持通过环境变量 QUANT_COMPRESSOR_TESTCASE_PATH 指定单条用例文件，实现进程级隔离执行：
#   - 设置时：仅运行该条用例（配合 batch_isolated_run.sh 每条用例拉起独立进程）
#   - 未设置：回退为原有行为，一次性加载目录下全部用例
_single_case_path = os.environ.get("QUANT_COMPRESSOR_TESTCASE_PATH", "").strip()
...
if _single_case_path:
    if not os.path.isfile(_single_case_path):
        print(
            f"错误: 环境变量 QUANT_COMPRESSOR_TESTCASE_PATH 指定的用例文件不存在: {_single_case_path}"
        )
    else:
...
def compressor(testcase_files):  # 初始化参数和tensor
    ret = quant_compressor_pt_loadprocess.test_compressor_process(
        testcase_files, device_id=0
    )
    cpu_result = ret["cpu_result"]
    kv_mask_result = ret["kv_mask_result"]
    npu_result = ret["npu_result"]
    cpu_kv_state = ret["cpu_kv_state"]
    mask_cpu_kv_state = ret["mask_cpu_kv_state"]
    cpu_score_state = ret["cpu_score_state"]
    mask_cpu_score_state = ret["mask_cpu_score_state"]
    state_cache = ret["state_cache"]
    params = ret["params"]
