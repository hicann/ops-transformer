...
import os
from quant_compressor_golden import cpu_compressor, get_seq_used_by_batch
from hif8_utils import sample_hif8_value_uniform
import pandas as pd
import numpy as np
import torch
import pytest
...
def load_excel_test_cases(excel_file_path: str, sheetname: str):
    ...
    try:
        ...
        required_columns = [
            ...
            "x_descale_datarange",
            "wkv_descale_datarange",
            "wgate_descale_datarange",
            "quant_mode",
        ]
        ...
        for _, row in df.iterrows():
            test_cases.append(
                (
                    ...
                    row["x_descale_datarange"],
                    row["wkv_descale_datarange"],
                    row["wgate_descale_datarange"],
                    row["quant_mode"],
                )
            )
...
def compressor_output_single(data_case):
    ...
    x_descale_datarange = [float(x.strip()) for x in x_descale_datarange.split(",")]
    wkv_descale_datarange = [float(x.strip()) for x in wkv_descale_datarange.split(",")]
    wgate_descale_datarange = [
        float(x.strip()) for x in wgate_descale_datarange.split(",")
    ]
    ...
    cpu_result, kv_mask_result = cpu_compressor(
        x,
        wkv,
        wgate,
        x_descale,
