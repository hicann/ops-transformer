...
_HIF8_UTILS_MODULE = None


def _load_hif8_utils():
    """Load tests/pytest/hif8_utils.py for HIF8 quantization helpers."""
    global _HIF8_UTILS_MODULE
    if _HIF8_UTILS_MODULE is not None:
        return _HIF8_UTILS_MODULE
    import importlib.util
    import sys
    from pathlib import Path

    pytest_dir = Path(__file__).resolve().parents[2] / "pytest"
    module_path = pytest_dir / "hif8_utils.py"
    sys.path.insert(0, str(pytest_dir))
    try:
        spec = importlib.util.spec_from_file_location(
            f"quant_compressor_hif8_utils_{abs(hash(module_path))}", module_path
        )
        ...
        try:
            ...
            pass
    _HIF8_UTILS_MODULE = module
    return _HIF8_UTILS_MODULE


def _gen_slice_data(rng, lo, hi, shape, dtype):
    """Generate seeded slice data. For uint8 (HIF8) tensors, convert float to HIF8 codes."""
    data = rng.uniform(lo, hi, size=tuple(shape)).astype(np.float32)
    if dtype == torch.uint8:
        hif8_utils = _load_hif8_utils()
        data = hif8_utils.fp32_to_hif8_numpy(data).astype(np.uint8)
    return data
...
def generate_quant_compressor_inputs(
    x,
    wkv,
    wgate,
    state_cache,
    ...
    **kwargs,
    ...
    state_cache = _ensure_torch(state_cache)
    ape = _ensure_torch(ape)
    x_descale = _ensure_torch(x_descale)
    wkv_descale = _ensure_torch(wkv_descale)
    wgate_descale = _ensure_torch(wgate_descale)
    state_block_table = _ensure_torch(state_block_table)
    cu_seqlens = _ensure_torch(cu_seqlens)
    seqused = _ensure_torch(seqused)
...
def aclnn_quant_compressor_input(
    x,
    wkv,
    wgate,
    stateCacheRef,
    ...
    **kwargs,
    ...
    wkv = _ensure_torch(wkv)
    wgate = _ensure_torch(wgate)
    stateCacheRef = _ensure_torch(stateCacheRef)
    ape = _ensure_torch(ape)
    xDescale = _ensure_torch(xDescale)
    wkvDescale = _ensure_torch(wkvDescale)
    wgateDescale = _ensure_torch(wgateDescale)
    stateBlockTable = _ensure_torch(stateBlockTable)
    cuSeqlens = _ensure_torch(cuSeqlens)
    seqused = _ensure_torch(seqused)
    startPos = _ensure_torch(startPos)
    cmpKv = _ensure_torch(cmpKv)
    cu_seqlens_list, seqused_list, start_pos_list, B, S, S_max, block_size = (
        _fill_seq_descriptors(
            x,
            stateCacheRef,
