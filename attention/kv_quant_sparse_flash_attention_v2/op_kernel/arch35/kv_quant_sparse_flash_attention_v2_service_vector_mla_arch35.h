...
/*!
 * \file kv_quant_sparse_flash_attention_v2_service_vector_mla_arch35.h
 * \brief
 */
#ifndef KV_QUANT_SPARSE_FLASH_ATTENTION_V2_SERVICE_VECTOR_MLA_ARCH35_H
#define KV_QUANT_SPARSE_FLASH_ATTENTION_V2_SERVICE_VECTOR_MLA_ARCH35_H

#include <limits>
#include "kv_quant_sparse_flash_attention_v2_common_arch35.h"
...
public:
    ...
    __aicore__ inline void InitVecBlock(TPipe *pipe,
                                        const KvQuantSparseFlashAttentionV2TilingDataMla *__restrict tiling,
...
private:
    ...
    TPipe *tPipe;
    const KvQuantSparseFlashAttentionV2TilingDataMla *__restrict tilingData;
    ...
    if (constInfo.returnSoftmaxLse) {
        ...
        if (coreNum != 0 && totalLseSize > 0) {
            ...
            if (constInfo.aivIdx * singleCoreLseSize < totalLseSize && singleInitLseSize > 0) {
                WaitFlag<AscendC::HardEvent::MTE3_V>(initOutputEventId);
                matmul::InitOutput<float>(this->softmaxSumGm[constInfo.aivIdx * singleCoreLseSize], singleInitLseSize,
                                          0);
                SetFlag<AscendC::HardEvent::MTE3_V>(initOutputEventId);
                WaitFlag<AscendC::HardEvent::MTE3_V>(initOutputEventId);
                // softmax_max init to -inf (QSFA specific, different from SFA which uses 0)
                matmul::InitOutput<float>(this->softmaxMaxGm[constInfo.aivIdx * singleCoreLseSize], singleInitLseSize,
...
#endif // KV_QUANT_SPARSE_FLASH_ATTENTION_V2_SERVICE_VECTOR_MLA_ARCH35_H
