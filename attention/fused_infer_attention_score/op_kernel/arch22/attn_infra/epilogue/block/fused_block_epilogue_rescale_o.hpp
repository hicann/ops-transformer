...
public:
    ...
    __aicore__ inline void CopyOToGm(AscendC::GlobalTensor<ElementOutput> gOutput, uint32_t proTokenIdx,
                                     ...
                                     uint32_t qSThisSubBlock, uint32_t embedValue, uint32_t embedRoundV,
                                     uint32_t oHiddenSize)
    {
        ...
        if (proTokenNum != 0U) {
            AscendC::DataCopyPad(gOutput[innerOGmOffset + proTokenIdx * oHiddenSize], goUbTensor16[innerGOUbOffset],
                                 AscendC::DataCopyExtParams(proTokenNum, embedValue * SIZE_OF_16BIT, 0,
                                                            (oHiddenSize - embedValue) * SIZE_OF_16BIT, 0));
        ...
        for (uint32_t qN_idx = 0; qN_idx < integralHeadNum; qN_idx++) {
            AscendC::DataCopyPad(gOutput[innerOGmOffset], goUbTensor16[innerGOUbOffset],
                                 AscendC::DataCopyExtParams(qSThisSubBlock, embedValue * SIZE_OF_16BIT, 0,
                                                            (oHiddenSize - embedValue) * SIZE_OF_16BIT, 0));
            innerOGmOffset += embedValue;
        ...
        if (epiTokenNum != 0U) {
            AscendC::DataCopyPad(gOutput[innerOGmOffset], goUbTensor16[innerGOUbOffset],
                                 AscendC::DataCopyExtParams(epiTokenNum, embedValue * SIZE_OF_16BIT, 0,
                                                            (oHiddenSize - embedValue) * SIZE_OF_16BIT, 0));
        }
    }
    ...
    __aicore__ inline void CopyOToGmFp32(AscendC::GlobalTensor<float> gOutput, uint32_t proTokenIdx,
                                         uint32_t proTokenNum, uint32_t epiTokenNum, uint32_t integralHeadNum,
                                         uint32_t qSThisSubBlock, uint32_t embedV, uint32_t embedRoundV,
        ...
        if (proTokenNum != 0U) {
            AscendC::DataCopyPad(gOutput[innerOGmOffset + proTokenIdx * oHiddenSize_gmlo],
                                 ...
                                 AscendC::DataCopyExtParams(proTokenNum, blockLen, srcStride,
                                                            (oHiddenSize_gmlo - embedV) * sizeof(float), 0));
    ...
    __aicore__ inline void SubCoreCompute(AscendC::GlobalTensor<ElementOutput> gOutput,
                                          ...
                                          uint32_t totalRowNum, uint32_t isFirstStackTile, uint32_t LastStackTileFlag,
                                          uint32_t curStackTileMod, uint32_t needRowLoopFlag, uint32_t isLastRowLoop,
    ...
    __aicore__ inline void SubCoreCompute(
        ...
        uint32_t LastRowLoopFlag, uint32_t rowOffsetLoop, uint32_t proTokenIdx, uint32_t proTokenNum,
        uint32_t epiTokenNum, uint32_t integralHeadNum, uint32_t rowOffsetCurLoop, int32_t delStartRow,
        int32_t delEndRow, uint32_t qSeqlen, uint32_t qSBlockIdx, uint32_t rowNum, uint32_t inRowOffsetThisSubBlock,
        ...
        if (!isFirstStackTile) {
            ...
            for (uint32_t vmul_idx = 0; vmul_idx < embedV / FLOAT_VECTOR_SIZE; ++vmul_idx) {
                AscendC::Mul<float, false>(goUbTensor32[vmul_idx * FLOAT_VECTOR_SIZE],
                                           goUbTensor32[vmul_idx * FLOAT_VECTOR_SIZE], tvUbTensor, (uint64_t)0,
                                           curRowNum,
                                           AscendC::BinaryRepeatParams(1, 1, 0, embedRoundV / FLOAT_BLOCK_SIZE,
                                                                       embedRoundV / FLOAT_BLOCK_SIZE, 1));
            }
            if (embedV % FLOAT_VECTOR_SIZE > 0) {
                ...
                AscendC::Mul<float, false>(goUbTensor32[embedV / FLOAT_VECTOR_SIZE * FLOAT_VECTOR_SIZE],
                                           goUbTensor32[embedV / FLOAT_VECTOR_SIZE * FLOAT_VECTOR_SIZE], tvUbTensor,
                                           (uint64_t)0, curRowNum,
                                           AscendC::BinaryRepeatParams(1, 1, 0, embedRoundV / FLOAT_BLOCK_SIZE,
                                                                       embedRoundV / FLOAT_BLOCK_SIZE, 1));
                AscendC::SetVectorMask<int8_t>((uint64_t)(-1), (uint64_t)(-1));
        ...
        if (isLastStackTile) {
            ...
            if (delEndRow != qSeqlen) {
                if (proTokenNum != 0U && subBlockStart < delEndRow) {
                    ...
                    AscendC::PipeBarrier<PIPE_V>();
                    AscendC::Duplicate<ElementOutput>(goUbTensor16[innerGOUbOffset], static_cast<ElementOutput>(0),
    ...
    __aicore__ inline void operator()(AscendC::GlobalTensor<ElementOutput> gOutput,
                                      ...
                                      uint32_t qNBlockSize, uint32_t kvNBlockSize, uint32_t isFirstStackTile,
                                      uint32_t isLastStackTile, uint32_t curStackTileMod, uint32_t isNew)
    {
        ...
        uint32_t inRowSplitSubBlock = (kvNBlockSize == 1U) ? ((qNBlockSize == 1U) ? (qSBlockSize / subBlockNum) :
        ...
        if (inRowActualThisSubBlock > 0U) {
            ...
            for (uint32_t rowLoopIdx = 0; rowLoopIdx < rowLoop; rowLoopIdx++) {
                ...
                SubCoreCompute(gOutputCurLoop, gInputCurLoop, gUpdateCurLoop, gLseThisSubBlock, layoutOutputCurLoop,
                               ...
                               curStackTileMod, needRowLoop, (rowLoopIdx == rowLoop - 1U), rowOffsetLoop, proTokenIndex,
                               proTokenNum, epiTokenNum, integralHeadNum);
            }
        }
    }
    ...
    __aicore__ inline void operator()(AscendC::GlobalTensor<ElementOutput> gOutput,
                                      AscendC::GlobalTensor<ElementInput> gInput,
                                      AscendC::GlobalTensor<ElementUpdate> gUpdate,
                                      AscendC::GlobalTensor<ElementLse> gLseOutput, const LayoutOutput &layoutOutput,
                                      const LayoutInput &layoutInput, const LayoutUpdate &layoutUpdate,
                                      const LayoutLse &layoutLse, GemmCoord actualBlockShape, uint32_t qSBlockSize,
                                      uint32_t qNBlockSize, uint32_t isFirstStackTile, uint32_t isLastStackTile,
                                      uint32_t curStackTileMod, int32_t delStartRow, int32_t delEndRow,
                                      uint32_t qSeqlen, uint32_t qSBlockIdx, uint32_t curQNBlockTile,
        ...
        if (inRowActualThisSubBlock > 0U) {
            ...
            for (uint32_t rowLoopIdx = 0; rowLoopIdx < rowLoop; rowLoopIdx++) {
                ...
                SubCoreCompute(gOutputCurLoop, gInputCurLoop, gUpdateCurLoop, gLseThisSubBlock, layoutOutputCurLoop,
                               ...
                               proTokenNum, epiTokenNum, integralHeadNum, rowOffsetCurLoop, delStartRow, delEndRow,
                               qSeqlen, qSBlockIdx, rowNum, inRowOffsetThisSubBlock, blockParams, curQNBlockTile);
            }
        }
    }
