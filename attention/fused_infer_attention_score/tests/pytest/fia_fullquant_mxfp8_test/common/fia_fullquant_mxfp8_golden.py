...
try:
    ...
    from precision_compare_v2 import (
        check_result,
        check_result_cv,
        compute_cv_report,
        display_cv_report,
...
USE_FP64_COMPARE = (
    True  # FP64 golden时, 用FP64做golden(最高精度)还是用FP32做golden(CV分母)
)
...
def cpu_mxfp8_golden_fp64(
    ...
    if N_q != N_kv:
        logger.info("[INFO] GQA broadcast (fp64)")
        k_tensor = broadcast_kv(N_q, N_kv, k_tensor)
        v_tensor = broadcast_kv(N_q, N_kv, v_tensor)
        dequant_scale_k = broadcast_kv(N_q, N_kv, dequant_scale_k)
        dequant_scale_v = broadcast_kv(N_q, N_kv, dequant_scale_v)
    ...
    o_max = (
        torch.ones(q_tensor.shape[:-1], dtype=torch.float64)[..., None]
    ...
    for i in range(TILES_Q):
        ...
        for j in range(0, TILES_KV, 2):
            ...
            if Qri is not None and Krj is not None:
                S_ij += torch.matmul(
                    Qri.to(torch.float64), Krj.to(torch.float64).permute(0, 1, 3, 2)
                )
            ...
            if j + 1 < TILES_KV:
                ...
                if Qri is not None and Krj1 is not None:
                    S_ij1 += torch.matmul(
                        Qri.to(torch.float64),
                        Krj1.to(torch.float64).permute(0, 1, 3, 2),
                    )
...
if __name__ == "__main__":
    ...
    if USE_FP64_GOLDEN:
        if "cpu" in mode:
            logger.info("\n[Step 2] CPU Golden")
            logger.info(
                "Mode: FP64 CV three-way comparison (golden_fp64 | golden_fp32 | NPU)"
            )
            ...
            cpu_out_fp32, cpu_lse_fp32 = cpu_mxfp8_golden(
                q_fp8,
                k_fp8,
                v_fp8,
                dequant_scale_q,
        ...
        logger.info("\n[Step 4] Atten OUT CV three-way comparison")
        golden_tnd = convert_q_bnsd_to_layout(
            cpu_out_fp64 if USE_FP64_COMPARE else cpu_out_fp32, ACTUAL_SEQ_Q, "TND"
        )
        ...
        if ENABLE_LSE:
            logger.info("\n[Step 5] LSE CV three-way comparison")
        ...
        logger.info("\n[Compare] Also output simple mode results (FP64 golden vs NPU):")
