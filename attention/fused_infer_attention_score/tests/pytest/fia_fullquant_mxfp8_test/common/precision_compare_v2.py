...
BM_CMP_STD_DEFAULT: Dict[str, Dict[str, float]] = {
    "fp32": {"max_re_rtol": 10.0, "avg_re_rtol": 2.0, "rmse_rtol": 2.0,
             "small_value": 0.000001, "small_value_atol": 0.000000},
    ...
    "bf16": {"max_re_rtol": 10.0, "avg_re_rtol": 2.0, "rmse_rtol": 2.0,
             "small_value": 0.0000001, "small_value_atol": 0.004},
    "hf32": {"max_re_rtol": 10.0, "avg_re_rtol": 2.0, "rmse_rtol": 2.0,
             "small_value": 0.000001, "small_value_atol": 0.000000},
}
...
def _normalize_dtype_key(dtype) -> str:
    ...
    s = str(dtype).lower()
    mapping = {
        "torch.float16": "fp16", "float16": "fp16", "fp16": "fp16",
        "torch.bfloat16": "bf16", "bfloat16": "bf16", "bf16": "bf16",
        "torch.float32": "fp32", "float32": "fp32", "fp32": "fp32",
        "torch.float64": "fp32", "float64": "fp32", "fp64": "fp32",
        "torch.half": "fp16",
        "torch.float8_e4m3fn": "fp16", "torch.float8_e5m2": "fp16",
        "float8_e4m3fn": "fp16", "float8_e5m2": "fp16",
        "torch_npu.float8_e8m0fnu": "fp16", "float8_e8m0": "fp16",
        "torch_npu.hifloat8": "fp16", "hifloat8": "fp16",
        "torch.int8": "fp16", "int8": "fp16",
    }
...
class Result:
    ...
    def __init__(self, result_name, total_big_num, total_big_ratio,
                 diff_big_max, diff_big_avg, diff_big_sum,
                 err_w1_num, err_w1_ratio, err_k1_num, err_k1_ratio,
                 err_k5_num, err_k5_ratio, err_h1_num, err_h1_ratio,
                 total_small_num, total_small_ratio, err_small_num, err_small_ratio,
                 diff_rmse, rst_eb, diff_eb,
                 diff_big_ratio_max, diff_big_ratio_avg, diff_big_ratio_rmse,
                 bm_cmp_std,
                 num_total_nan=0, err_total_nan=0,
                 num_total_inf=0, err_total_inf=0,
                 num_total_ninf=0, err_total_ninf=0):
    ...
    def print_result(self):
        logger.info(f"---- 结果: {self.result_name} ----")
        logger.info(f" 大值总数: {self.total_big_num}")
        logger.info(f" 大值占比: {self.total_big_ratio:.2%}")
        logger.info(f" 大值最大绝对误差: {self.diff_big_max:.8f}")
        logger.info(f" 大值平均绝对误差: {self.diff_big_avg:.8f}")
        logger.info(f" 大值绝对误差总和: {self.diff_big_sum:.2f}")
        logger.info(f" 大值最大相对误差: {self.diff_big_ratio_max:.8f}")
        logger.info(f" 大值平均相对误差: {self.diff_big_ratio_avg:.8f}")
        logger.info(f" 大值相对误差均方根 (RMSE): {self.diff_big_ratio_rmse:.8f}")
        logger.info(f" 大值万分之1误差数: {self.err_w1_num}, 占比 {self.err_w1_ratio:.2%}")
        logger.info(f" 大值千分之1误差数: {self.err_k1_num}, 占比 {self.err_k1_ratio:.2%}")
        logger.info(f" 大值千分之5误差数: {self.err_k5_num}, 占比 {self.err_k5_ratio:.2%}")
        logger.info(f" 大值百分之1误差数: {self.err_h1_num}, 占比 {self.err_h1_ratio:.2%}")
        logger.info(f" 小值总数: {self.total_small_num}")
        logger.info(f" 小值占比: {self.total_small_ratio:.2%}")
        logger.info(f" 小值错误数: {self.err_small_num}, 占比 {self.err_small_ratio:.2%}")
        logger.info(f" 误差均方根 (RMSE): {self.diff_rmse:.8f}")
        logger.info(f" 均衡性偏差计数: {self.rst_eb}")
        logger.info(f" 均衡性 diff 总和: {self.diff_eb:.8f}")
        logger.info(f" golden nan 总数: {self.num_total_nan}, nan 误差数: {self.err_total_nan}")
        logger.info(f" golden inf 总数: {self.num_total_inf}, inf 误差数: {self.err_total_inf}")
        logger.info(f" golden -inf 总数: {self.num_total_ninf}, -inf 误差数: {self.err_total_ninf}")
    ...
    def check_result_debug(self, benchmark, new=False, output_dtype='fp16'):
        ...
        if new:
            ...
            benchmark.diff_big_ratio_max = benchmark.diff_big_ratio_max.maximum(lo_tensor)
            benchmark.diff_big_ratio_avg = benchmark.diff_big_ratio_avg.maximum(lo_tensor)
    ...
    def check_result(self, benchmark, new=False, output_dtype='fp16'):
        ...
        if 'error' in reason_str:
            logger.info(f"{self.result_name} compare result: error")
            return 'Failed', reason_str
        elif 'warning' in reason_str:
            logger.info(f"{self.result_name} compare result: warning")
...
def compute_result(value: torch.Tensor, golden: torch.Tensor,
                   name: str, output_dtype: str,
                   bm_cmp_std: Optional[Dict[str, float]] = None,
                   red_list: Optional[List[float]] = None) -> Optional[Result]:
    ...
    if value.shape != golden.shape:
        logger.error(f"error: {name} shape mismatch, value={value.shape} golden={golden.shape}")
        return None
    ...
    total_big_ratio = float(total_big_num / golden.numel()) if golden.numel() > 0 else 0.0
    ...
    diff_big_avg = diff_big_sum / total_big_num if total_big_num > 0 else torch.tensor(0.0)

    diff_big_ratio = diff_big / golden_big.abs()
    diff_big_ratio_max = diff_big_ratio.max() if diff_big_ratio.numel() > 0 else torch.tensor(0.0)
    diff_big_ratio_avg = diff_big_ratio.sum() / total_big_num if total_big_num > 0 else torch.tensor(0.0)
    diff_big_ratio_rmse = torch.sqrt(torch.mean(torch.square(diff_big_ratio))) if diff_big_ratio.numel() > 0 else torch.tensor(0.0)

    err_w1_num = int(torch.sum(diff_big_ratio > red_list[0]).item())
    err_w1_ratio = err_w1_num / total_big_num if total_big_num > 0 else torch.tensor(0.0)
    err_k1_num = int(torch.sum(diff_big_ratio > red_list[1]).item())
    err_k1_ratio = err_k1_num / total_big_num if total_big_num > 0 else torch.tensor(0.0)
    err_k5_num = int(torch.sum(diff_big_ratio > red_list[2]).item())
    err_k5_ratio = err_k5_num / total_big_num if total_big_num > 0 else torch.tensor(0.0)
    err_h1_num = int(torch.sum(diff_big_ratio > red_list[3]).item())
    err_h1_ratio = err_h1_num / total_big_num if total_big_num > 0 else torch.tensor(0.0)

    # ---- 小值对比 ----
    total_small_num = int(torch.sum(golden.abs() < bm_cmp_std['small_value']).item())
    total_small_ratio = total_small_num / golden.numel() if golden.numel() > 0 else torch.tensor(0.0)
    ...
    err_small_ratio = err_small_num / total_small_num if total_small_num > 0 else torch.tensor(0.0)

    # ---- 全局 RMSE (method=12 用 diff_big_rmse 覆写, 这里保持一致) ----
    diff_big_rmse = torch.sqrt(torch.mean(torch.square(diff_big))) if diff_big.numel() > 0 else torch.tensor(0.0)
    ...
    return Result(
        name, total_big_num, total_big_ratio, diff_big_max, diff_big_avg, diff_big_sum,
        err_w1_num, err_w1_ratio, err_k1_num, err_k1_ratio, err_k5_num, err_k5_ratio,
        err_h1_num, err_h1_ratio,
        total_small_num, total_small_ratio, err_small_num, err_small_ratio,
        diff_rmse, rst_eb, diff_eb,
        diff_big_ratio_max, diff_big_ratio_avg, diff_big_ratio_rmse, bm_cmp_std,
        num_total_nan, err_total_nan, num_total_inf, err_total_inf,
        num_total_ninf, err_total_ninf,
    )
...
def data_compare_benchmark_new(
    ...
    if real_data.size == 0 and (data_compe is None or real_data.size == data_compe.size) and real_data.size == cpu_golden.size:
        logger.info("The npu_output is [], and it matches bm/golden, result=Pass")
        return "Pass", "", []

    if real_data.size != cpu_golden.size:
        logger.error(f"Error, size of npu output[{real_data.size}] and golden[{cpu_golden.size}] not equal.")
        return "Failed", "size mismatch", []

    if data_compe is not None and real_data.size != data_compe.size:
        logger.error(f"Error, size of npu output[{real_data.size}] and benchmark[{data_compe.size}] not equal.")
        return "Failed", "size mismatch", []
    ...
    # NPU vs golden
    rst_npu = compute_result(npu_res, golden_t, test_name + "_npu", output_dtype, bm_cmp_std, red_list)
    if rst_npu is None:
    ...
    if data_compe is not None:
        ...
        rst_gpu = compute_result(benchmark_res, golden_t.clone(), test_name + "_bench", output_dtype, bm_cmp_std, red_list)
        if rst_gpu is None:
            return "Failed", "bench shape mismatch", []
        rst_gpu.print_result()
        # new=True: 下界抬高 + 去掉 diff_big_sum / diff_big_ratio_rmse 判等
        status, reason = rst_npu.check_result(rst_gpu, new=True, output_dtype=output_dtype)
    else:
    ...
    data = [
        test_name, test_name,
        f"{rst_npu.total_big_num}", f"{rst_npu.total_big_ratio:.2%}",
        f"{rst_npu.err_w1_ratio:.2%}", f"{rst_npu.err_k1_ratio:.2%}",
        f"{rst_npu.err_k5_ratio:.2%}", f"{rst_npu.err_h1_ratio:.2%}",
        ...
        f"{rst_npu.total_small_num}", f"{rst_npu.total_small_ratio:.2%}",
        f"{rst_npu.err_small_num}", f"{rst_npu.err_small_ratio:.2%}",
        f"{rst_npu.num_total_nan:.2f}", f"{rst_npu.err_total_nan:.2f}",
        f"{rst_npu.num_total_inf:.2f}", f"{rst_npu.err_total_inf:.2f}",
        f"{rst_npu.num_total_ninf:.2f}", f"{rst_npu.err_total_ninf:.2f}",
        f"{rst_npu.diff_rmse:.8f}", f"{rst_npu.rst_eb}", f"{rst_npu.diff_eb:.8f}",
        f"{rst_gpu.total_big_num}", f"{rst_gpu.total_big_ratio:.2%}",
        f"{rst_gpu.err_w1_ratio:.2%}", f"{rst_gpu.err_k1_ratio:.2%}",
        f"{rst_gpu.err_k5_ratio:.2%}", f"{rst_gpu.err_h1_ratio:.2%}",
        ...
        f"{rst_gpu.total_small_num}", f"{rst_gpu.total_small_ratio:.2%}",
        f"{rst_gpu.err_small_num}", f"{rst_gpu.err_small_ratio:.2%}",
        f"{rst_gpu.num_total_nan:.2f}", f"{rst_gpu.err_total_nan:.2f}",
        f"{rst_gpu.num_total_inf:.2f}", f"{rst_gpu.err_total_inf:.2f}",
        f"{rst_gpu.num_total_ninf:.2f}", f"{rst_gpu.err_total_ninf:.2f}",
        f"{rst_gpu.diff_rmse:.8f}", f"{rst_gpu.rst_eb}", f"{rst_gpu.diff_eb:.8f}",
        f"{status}", f"{reason}",
    ]
...
def compute_cv_report(
    ...
    report = CVPrecisionReport(
        test_name=test_name, level="L1",
        output_dtype=output_dtype,
        bm_cmp_std=bm_cmp_std, red_list=red_list,
    )
    ...
    if real_data.size == 0 and real_data.size == golden_flat.size and \
            (bench_flat is None or real_data.size == bench_flat.size):
        report.passed = True
        report.status = "Pass"
        return report
    ...
    if bench_flat is not None:
        ...
        rst_bench = compute_result(bench_res, golden_t.clone(), test_name + "_bench",
                                    output_dtype, bm_cmp_std, red_list)
        ...
        # new=True: 下界抬高 + 去掉 diff_big_sum / diff_big_ratio_rmse 判等
        status, reason = rst_npu.check_result(rst_bench, new=True, output_dtype=output_dtype)
    else:
    ...
    if reason:
        report.failure_reasons = [r.strip().rstrip('/') for r in reason.split('/') if r.strip()]
    return report
...
def display_cv_report(report: CVPrecisionReport):
    ...
    SUB = "-" * 72
    logger.info(SEP)
    logger.info("  算子精度比对报告 (method=12 风格)")
    if report.test_name:
    ...
    logger.info(f"  [bm_cmp_std]  max_re_rtol={bm.get('max_re_rtol', 0):.1f} "
    ...
    logger.info(f"  [red_range]   w1={report.red_list[0]:g} k1={report.red_list[1]:g} "
                f"k5={report.red_list[2]:g} h1={report.red_list[3]:g}")

    def _print_one(label, rst: Result):
        logger.info(SUB)
        logger.info(f"  [{label}]")
        logger.info(f"    大值: num={rst.total_big_num} ratio={rst.total_big_ratio:.2%} "
                    f"max_abs={rst.diff_big_max:.8f} avg_abs={rst.diff_big_avg:.8f} sum={rst.diff_big_sum:.2f}")
        logger.info(f"    大值相对误差: max={rst.diff_big_ratio_max:.8f} "
                    f"avg={rst.diff_big_ratio_avg:.8f} rmse={rst.diff_big_ratio_rmse:.8f}")
        logger.info(f"    大值误差占比: w1={rst.err_w1_ratio:.2%} k1={rst.err_k1_ratio:.2%} "
                    f"k5={rst.err_k5_ratio:.2%} h1={rst.err_h1_ratio:.2%}")
        logger.info(f"    小值: num={rst.total_small_num} ratio={rst.total_small_ratio:.2%} "
                    f"err_num={rst.err_small_num} err_ratio={rst.err_small_ratio:.2%}")
        logger.info(f"    RMSE={rst.diff_rmse:.8f}  eb={rst.rst_eb}  diff_eb={rst.diff_eb:.8f}")
        logger.info(f"    nan: total={rst.num_total_nan} err={rst.err_total_nan} | "
...
def check_result_cv(
    ...
    cv = compute_cv_report(
        golden=golden, benchmark=benchmark, actual=npu_result,
        test_name=test_name, output_dtype=output_dtype,
...
def check_result(
    ...
    if output_dtype is None:
        output_dtype = _normalize_dtype_key(npu_result.dtype if isinstance(npu_result, torch.Tensor) else "fp16")

    cv = compute_cv_report(
        golden=expect, benchmark=benchmark, actual=npu_result,
        test_name="", output_dtype=output_dtype,
        input_dtype_hint=input_dtype_hint,
    )
    if verbose:
        display_cv_report(cv)
...
def compare_multi_output(
    ...
    for name, (golden, benchmark, actual) in outputs.items():
        reports[name] = compute_cv_report(
            golden=golden, benchmark=benchmark, actual=actual,
            test_name=name, output_dtype=output_dtype,
            input_dtype_hint=input_dtype_hint,
        )
    return reports
