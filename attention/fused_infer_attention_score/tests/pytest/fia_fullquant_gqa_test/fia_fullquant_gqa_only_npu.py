...
def create_block_table(actual_seq_kv, block_size, seed=SEED_BLOCK_TABLE, run_idx=0):
    ...
    block_idx_list = np.random.default_rng(seed + run_idx).permutation(np.arange(total_blocks, dtype=np.int32))
...
def generate_data(run_idx=0):
    ...
    print(f"[INFO] [run {run_idx}] max_sq={max_sq}, max_skv={max_skv}")
    ...
    print(f"[INFO] [run {run_idx}] q_fp8 shape: {q_fp8.shape}, dtype: {q_fp8.dtype}")
    print(f"[INFO] [run {run_idx}] k_fp8 shape: {k_fp8.shape}, dtype: {k_fp8.dtype}")
    print(f"[INFO] [run {run_idx}] v_fp8 shape: {v_fp8.shape}, dtype: {v_fp8.dtype}")
...
def npu_fp8_full_quant(q_fp8, k_fp8, v_fp8,
                       dequant_scale_q, dequant_scale_k, dequant_scale_v, p_scale,
                       actual_seq_q, actual_seq_kv, run_idx=0):
...
if __name__ == '__main__':
    print("=" * 60)
    print("FIA Full Quant GQA NPU ({} runs, refresh input each run)".format(RUN_TIMES))
    print("=" * 60)
    ...
    for run_idx in range(RUN_TIMES):
        print("\n" + "=" * 60)
        print(f"[Run {run_idx + 1}/{RUN_TIMES}] 开始")
        print("=" * 60)

        print(f"\n[Run {run_idx + 1} Step 1] 数据生成 (seed 偏移 run_idx={run_idx})")
        (q_fp8, k_fp8, v_fp8, dequant_scale_q, dequant_scale_k, dequant_scale_v, p_scale) = generate_data(run_idx)

        print(f"\n[Run {run_idx + 1} Step 2] NPU: 执行NPU计算")
        npu_out = npu_fp8_full_quant(q_fp8, k_fp8, v_fp8,
                                     dequant_scale_q, dequant_scale_k, dequant_scale_v, p_scale,
                                     ACTUAL_SEQ_Q, ACTUAL_SEQ_KV, run_idx=run_idx)
        print(f"[INFO] [run {run_idx}] npu_out[0] shape: {npu_out[0].shape}, dtype: {npu_out[0].dtype}")
        if ENABLE_LSE:
            print(f"[INFO] [run {run_idx}] npu_out[1] shape: {npu_out[1].shape}, dtype: {npu_out[1].dtype}")
        print("#" * 60)

    print("\n" + "=" * 60)
    print(f"[INFO] 全部 {RUN_TIMES} 次运行完成")
    print("=" * 60)
