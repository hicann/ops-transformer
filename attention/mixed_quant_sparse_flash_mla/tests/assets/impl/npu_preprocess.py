...
# -----------------------------------------------------------------------------------------------------------
"""Populate MixedQuantSparseFlashMla metadata without a TTK context."""
...
def build_metadata_arguments(q, ori_kv, cmp_kv, quant_mode, kwargs):
    ...
    return {
        ...
        "rope_head_dim": int(get_attribute(kwargs, "rope_head_dim", 0)),
        "cu_seqlens_q": cu_q,
...
def run_metadata(arguments, metadata):
    ...
    return torch.ops.cann_ops_transformer.mixed_quant_sparse_flash_mla_metadata(
        ...
        rope_head_dim=int(arguments["rope_head_dim"]),
...
def run(q, *, ori_kv=None, cmp_kv=None, metadata=None, quant_mode=None, **kwargs):
    ...
    if metadata is None:
        raise ValueError("MixedQuantSparseFlashMla npu_preprocess requires metadata")
    ...
    if tuple(metadata.shape) != tuple(generated.shape):
        raise ValueError(
            "MQSMLA metadata shape mismatch: "
