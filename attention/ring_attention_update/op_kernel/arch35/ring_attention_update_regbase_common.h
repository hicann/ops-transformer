                                      ...
                                      LocalTensor<float> tempFp32Buf2Local, int64_t softmaxEleNumLoop, int64_t calNum)
{
    ...
    __VEC_SCOPE__
    {
        ...
        AscendC::Reg::RegTensor<float> outreg0;
        ...
        for (uint16_t i = 0; i < repeatTimes; i++) {
            ...
            AscendC::Reg::Exp(vreg4, vreg2, maskReg);
            ...
            AscendC::Reg::Mul(vreg9, vreg7, vreg5, maskReg);
    ...
    __VEC_SCOPE__
    {
        ...
        for (uint16_t idx = 0; idx < loop; idx++) { // 外循环
            ...
            for (uint16_t i = 0; i < repeatTimes; i++) {
                ...
                AscendC::Reg::Mul(vreg5, vreg1, vreg3, maskReg);
                                           ...
                                           int64_t headDimLoopEach, int64_t softmaxTailSize)
{
    ...
    __VEC_SCOPE__
    {
        ...
        AscendC::Reg::RegTensor<float> vreg7;
        AscendC::Reg::RegTensor<float> vreg8;
        AscendC::Reg::RegTensor<T> outreg0;
        ...
        for (uint16_t idx = 0; idx < loop; idx++) { // 外循环
            ...
            for (uint16_t i = 0; i < repeatTimes; i++) {
                ...
                // prev_attn_out * prev_out_scale
                AscendC::Reg::Mul(vreg6, vreg2, vreg3, maskReg);
                // cur_attn_out bf16/fp16 -> fp32
                AscendC::Reg::Cast<float, T, castTrait0>(vreg5, vreg1, maskReg);
                // cur_attn_out * cur_out_scale
                AscendC::Reg::Mul(vreg7, vreg4, vreg5, maskReg);
                // cur_attn_out * cur_out_scale + prev_attn_out * prev_out_scale
                AscendC::Reg::Add(vreg8, vreg6, vreg7, maskReg);
