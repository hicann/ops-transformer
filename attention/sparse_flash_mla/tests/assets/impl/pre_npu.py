...
"""SparseFlashMla metadata stage between input restoration and the main API."""
...
def clone_metadata_value(value):
    ...
    if hasattr(value, "item"):
        return value.item()
    raise TypeError(
        f"unsupported SparseFlashMla metadata input type: {type(value).__name__}"
    )
...
def persist_metadata_inputs(testcase_name, metadata_input, context):
    ...
    if not isinstance(metadata_input, dict):
        raise ValueError("SparseFlashMla pytest data lacks metadata_input")
    ...
    if path.is_symlink() or (path.exists() and not path.is_file()):
        raise ValueError(f"SparseFlashMla metadata state is not a regular file: {path}")
...
def load_metadata_inputs(context):
    ...
    if payload is None and path is not None:
        if path.is_symlink():
            raise ValueError(
                f"SparseFlashMla metadata state is not a regular file: {path}"
            )
        if path.exists():
            if not path.is_file():
                raise ValueError(
    ...
    if (
        ...
        raise ValueError("SparseFlashMla metadata state has an incompatible schema")
    if payload.get("csv_contract") != csv_contract_digest(context):
        raise ValueError(
            "SparseFlashMla metadata state does not match the current CSV contract"
        )
    metadata_input = payload.get("metadata_input")
    if not isinstance(metadata_input, dict):
        raise ValueError("SparseFlashMla metadata state lacks metadata_input")
    context.state[METADATA_INPUTS_STATE_KEY] = payload
...
def metadata_arguments(context):
    ...
    if metadata_input is None:
        raise RuntimeError(
            "SparseFlashMla requires input-stage metadata inputs before pre-NPU"
        )
    return metadata_input
...
def copy_metadata(target, source):
    ...
    if tuple(target.shape) != tuple(source.shape):
        raise ValueError(
            f"SMLA metadata shape mismatch: placeholder={tuple(target.shape)}, "
...
def persist_metadata(context, metadata):
    ...
    if len(matches) != 1:
        raise RuntimeError(
            f"SMLA expected one stored metadata input, found {len(matches)} in {case_dir}"
        )
    path = matches[0]
    temporary = path.with_name(f".{path.name}.{os.getpid()}.tmp")
    array = (
    ...
    if file_format == "bin":
        ...
        raise ValueError(f"SMLA cannot persist manual-data format {file_format!r}")
    temporary.replace(path)
...
def run_torch(context, target, arguments):
    ...
    def operation():
        nonlocal generated
        generated = torch.ops.cann_ops_transformer.sparse_flash_mla_metadata(
            arguments["num_heads_q"],
            arguments["num_heads_kv"],
            arguments["head_dim"],
            cu_seqlens_q=to_npu(arguments["cu_seqlens_q"]),
            cu_seqlens_ori_kv=to_npu(arguments["cu_seqlens_ori_kv"]),
            cu_seqlens_cmp_kv=to_npu(arguments["cu_seqlens_cmp_kv"]),
    ...
    with path.open("a+", newline="", encoding="utf-8") as file:
        ...
        try:
            ...
            if header is not None and tuple(header) != METADATA_RESULT_FIELDS:
                raise RuntimeError(
                    f"SMLA metadata result header does not match: {path}"
                )
...
def run(*args, context: TtkContext = None, **kwargs):
    ...
    if context is None:
        raise RuntimeError(
            "SMLA metadata stage requires an explicitly injected context"
        )
    ...
    try:
        ...
        if len(tensors) <= METADATA_INDEX or tensors[METADATA_INDEX] is None:
            raise ValueError("SMLA CSV must reserve tensor slot 17 for metadata")
        ...
        if context.case_type == "aclnn":
            ...
            raise ValueError(
                f"SMLA pre-NPU does not support case type {context.case_type!r}"
            )
        # Explicit false is a one-run refresh: the main API consumes the updated
        # in-memory tensor, while the replay dataset remains unchanged.
        ...
        if stop_after_stage is not None:
            try:
                ...
                logging.exception("Failed to write SMLA metadata error result")
        raise
