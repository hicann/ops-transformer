...
class BatchOutputComparator:
    ...
    def normalize_shapes(self, shapes, dynamic_extents):
        ...
        for index, shape in enumerate(shapes):
            ...
            if index in self.PREFIX_LENGTH_INPUT_INDEXES:
                ...
                continue
            if value and value[0] in dynamic_extents:
    ...
    def normalize_ranges(self, ranges):
        ...
        for index, value in enumerate(ranges):
            # Prefix-length storage is preallocated to B + 1, so its range is not relation identity.
            normalized.append(None if index in self.PREFIX_LENGTH_INPUT_INDEXES else self.normalize_value(value))
        return normalized
    ...
    def dynamic_extents(batch_size, prefixes):
        ...
        return {
            extent: "/".join(sorted(value)) for extent, value in labels.items()
        }
    ...
    def build_samples(self, case_rows, result_rows):
        ...
        for row in case_rows:
            ...
            shapes = self.parse_cell(row, "tensor_view_shapes")
            dtypes = self.parse_cell(row, "tensor_dtypes")
            if self.output_index >= len(shapes) or self.output_index >= len(dtypes):
                raise ValueError(
                    f"{testcase_name}: output index {self.output_index} is outside tensor metadata"
                )
