...
"""QuantSparseFlashMla metadata stage between input restoration and the main API."""
...
def clone_metadata_value(value):
    ...
    if hasattr(value, "item"):
        return value.item()
    raise TypeError(
        f"unsupported QuantSparseFlashMla metadata input type: {type(value).__name__}"
    )
...
def persist_metadata_inputs(testcase_name, metadata_input, context):
    ...
    if path.is_symlink() or (path.exists() and not path.is_file()):
        raise ValueError(
            f"QuantSparseFlashMla metadata state is not a regular file: {path}"
        )
...
def load_metadata_inputs(context):
    ...
    if (
        ...
        raise ValueError(
            "QuantSparseFlashMla metadata state has an incompatible schema"
        )
    if payload.get("csv_contract") != csv_contract_digest(context):
        raise ValueError(
            "QuantSparseFlashMla metadata state does not match the current CSV contract"
        )
    metadata_input = payload.get("metadata_input")
    if not isinstance(metadata_input, dict):
        raise ValueError("QuantSparseFlashMla metadata state lacks metadata_input")
    context.state[METADATA_INPUTS_STATE_KEY] = payload
...
def metadata_arguments(context):
    ...
    if metadata_input is None:
        raise RuntimeError(
            "QuantSparseFlashMla requires input-stage metadata inputs before pre-NPU"
        )
    return metadata_input
...
def copy_metadata(target, source):
    ...
    if tuple(target.shape) != tuple(source.shape):
        raise ValueError(
            f"QSMLA metadata shape mismatch: placeholder={tuple(target.shape)}, "
...
def persist_metadata(context, metadata):
    ...
    if len(matches) != 1:
        raise RuntimeError(
            f"QSMLA expected one stored metadata input, found {len(matches)} in {case_dir}"
        )
    path = matches[0]
    temporary = path.with_name(f".{path.name}.{os.getpid()}.tmp")
    array = (
    ...
    if file_format == "bin":
        ...
        raise ValueError(f"QSMLA cannot persist manual-data format {file_format!r}")
    temporary.replace(path)
...
def run_torch(context, target, arguments):
    ...
    def operation():
        nonlocal generated
        generated = torch.ops.cann_ops_transformer.quant_sparse_flash_mla_metadata(
    ...
    with path.open("a+", newline="", encoding="utf-8") as file:
        ...
        try:
            ...
            if header is not None and tuple(header) != METADATA_RESULT_FIELDS:
                raise RuntimeError(
                    f"QSMLA metadata result header does not match: {path}"
                )
...
def run(*args, context: TtkContext = None, **kwargs):
    ...
    if context is None:
        raise RuntimeError(
            "QSMLA metadata stage requires an explicitly injected context"
        )
    ...
    try:
        ...
        if len(tensors) <= METADATA_INDEX or tensors[METADATA_INDEX] is None:
            raise ValueError("QSMLA CSV must reserve tensor slot 20 for metadata")
        ...
        if context.case_type == "aclnn":
            ...
            raise ValueError(
                f"QSMLA pre-NPU does not support case type {context.case_type!r}"
            )
        ...
        if stop_after_stage is not None:
            try:
                ...
                logging.exception("Failed to write QSMLA metadata error result")
        raise
