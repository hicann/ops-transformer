...
public:
    ...
    __aicore__ inline void SetFloatVecMask(int32_t len)
    {
        uint64_t mask = 0;
        uint64_t one = 1;
                                              ...
                                              uint32_t numElemsAligned)
    {
        ...
        uint32_t loopCount =
            numElemsAligned / HALF_VECTOR_SIZE; // half低精度场景，每行需要numElemsAligned/128次循环处理
        // 每个datablock长度32Byte，因此half低精度场景，每个datablock内有32/2=16个元素
        uint8_t dataBlockNumPerRow =
            numElemsAligned / BLOCK_SIZE; // half低精度场景，每行共有numElemsAligned/16个datablock
                                              ...
                                              uint32_t isFirstStackTile)
    {
        ...
        if (isFirstStackTile) {
            ...
            AscendC::PipeBarrier<PIPE_V>();
            // *** gm(float) = cast(hm)
            AscendC::Cast(gmUbTensor[rowOffset], hmUbTensor[rowOffset], AscendC::RoundMode::CAST_NONE,
                          rowNumCurLoopRound);
            AscendC::PipeBarrier<PIPE_V>();
        } else {
            // *** hm_f = cast(lm); hm_f = max(hm_f, gm); dm = exp(gm - hm_f); hm/gm = hm_f
            AscendC::Cast(tvF[rowOffset], lmUbTensor[rowOffset], AscendC::RoundMode::CAST_NONE, rowNumCurLoopRound);
            AscendC::PipeBarrier<PIPE_V>();
            SetFloatVecMask(rowNumCurLoop);
            AscendC::Max<float, false>(tvF[rowOffset], tvF[rowOffset], gmUbTensor[rowOffset], (uint64_t)0, 1,
                                       AscendC::BinaryRepeatParams(1, 1, 1, 8, 8, 8));
            AscendC::PipeBarrier<PIPE_V>();
            AscendC::Sub<float, false>(dmUbTensor[dmUbOffsetCurCycle], gmUbTensor[rowOffset], tvF[rowOffset],
            ...
            AscendC::SetVectorMask<int8_t>((uint64_t)-1, (uint64_t)-1);
            AscendC::PipeBarrier<PIPE_V>();
            AscendC::Cast(hmUbTensor[rowOffset], tvF[rowOffset], AscendC::RoundMode::CAST_NONE, rowNumCurLoopRound);
            AscendC::PipeBarrier<PIPE_V>();
            AscendC::DataCopy(gmUbTensor[rowOffset], tvF[rowOffset],
                                              ...
                                              uint32_t isFirstStackTile)
    {
        ...
        if (isFirstStackTile) {
            // *** gl(float) = cast(ll)
            AscendC::Cast(glUbTensor[rowOffset], llUbTensor[rowOffset], AscendC::RoundMode::CAST_NONE,
                          rowNumCurLoopRound);
            AscendC::PipeBarrier<PIPE_V>();
        } else {
            SetFloatVecMask(rowNumCurLoop);
            ...
            AscendC::PipeBarrier<PIPE_V>();
            AscendC::SetVectorMask<int8_t>((uint64_t)-1, (uint64_t)-1);
            AscendC::Cast(tvF[rowOffset], llUbTensor[rowOffset], AscendC::RoundMode::CAST_NONE, rowNumCurLoopRound);
            AscendC::PipeBarrier<PIPE_V>();
            SetFloatVecMask(rowNumCurLoop);
            // *** gl = gl + cast(ll)
            AscendC::Add<float, false>(glUbTensor[rowOffset], glUbTensor[rowOffset], tvF[rowOffset], (uint64_t)0, 1,
                                       AscendC::BinaryRepeatParams(1, 1, 1, 8, 8, 8));
            AscendC::PipeBarrier<PIPE_V>();
            AscendC::SetVectorMask<int8_t>((uint64_t)-1, (uint64_t)-1);
        }
                                      ...
                                      Arch::CrossCoreFlag softmaxFlag)
    {
        ...
        for (uint32_t rowLoopIdx = 0; rowLoopIdx < rowLoopNum; rowLoopIdx++) {
            ...
            SubCoreCompute(gOutputCurLoop, layoutOutputCurLoop, rowOffsetCurLoop, isFirstStackTile, (rowLoopIdx == 0),
                           columnNumRound, pingpongFlag, curStackTileMod, softmaxFlag, 0, 0);
        }
    }
