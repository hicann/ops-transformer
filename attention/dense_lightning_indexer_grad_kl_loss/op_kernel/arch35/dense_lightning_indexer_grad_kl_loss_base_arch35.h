...
/*!
 * \file dense_lightning_indexer_grad_kl_loss_base_arch35.h
 * \brief
 */

#ifndef DENSE_LIGHTNING_INDEXER_GRAD_KL_LOSS_BASE_ARCH35_H
#define DENSE_LIGHTNING_INDEXER_GRAD_KL_LOSS_BASE_ARCH35_H
...
#include "dense_lightning_indexer_grad_kl_loss_vector_arch35.h"
#include "dense_lightning_indexer_grad_kl_loss_vector2_arch35.h"
#include "dense_lightning_indexer_grad_kl_loss_service_cube_arch35.h"
...
private:
    ...
    DenseLightningIndexerGradKLLossServiceCube<DLIT> matmulService;
    DenseLightningIndexerGradKLLossVector<DLIT> vectorService;
    DenseLightningIndexerGradKLLossVector2<DLIT> vector2Service;
...
__aicore__ inline void DenseLightningIndexerGradKLLossBase<DLIT>::InitConstInfo()
{
    if ASCEND_IS_AIV {
        ...
        constInfo.aivIdx = constInfo.aicIdx * AIC_AIV_RATIO;
        constInfo.subBlockIdx = 0;
    }
    ...
    if ASCEND_IS_AIV {
        if (constInfo.aivIdx == 0) {
            ...
            AscendC::DataCacheCleanAndInvalid<T, AscendC::CacheLine::SINGLE_CACHE_LINE,
                                              AscendC::DcciDst::CACHELINE_OUT>(lossGm);
        }
    }
        ...
        for (int64_t s1Idx = s1StartIdxThisBatch; s1Idx < s1EndIdxThisBatch; s1Idx += S1_BASE_STEP) {
            ...
            for (int64_t s2Idx = 0; s2Idx < s2EndIdx + s2PreloadTail; s2Idx += S2_BASE_STEP) {
                ...
                if constexpr(deterministic) {
                    ...
                    SyncAll<false>();
                    if ASCEND_IS_AIV {
                        vectorService.DeterAddKIndexGrad();
                    }
                    SyncAll<false>();
                }
                if ASCEND_IS_AIV {
