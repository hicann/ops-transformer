...
def npu_qfa_mxfp8(q: torch.Tensor, k: torch.Tensor, v: torch.Tensor,
    ...
    if env_graph_path is not None:
        ...
        logger.info("[WRAPPER] 使用环境变量 GRAPH_PATH=%d (覆盖 CSV graph_path)", graph_path)
    ...
    if cached is not None:
        ...
        logger.warning("[WRAPPER] customize_inputs 缓存为空,使用 TTK 传入的占位 tensor(可能出错)")

    p_scale_val = float(p_scale.item()) if isinstance(p_scale, torch.Tensor) else float(p_scale)
    ...
    actual_seq_q = [cu_seqlens_q_list[i + 1] - cu_seqlens_q_list[i] for i in range(len(cu_seqlens_q_list) - 1)] if len(cu_seqlens_q_list) > 1 else [0]
    actual_seq_kv = [cu_seqlens_kv_list[i + 1] - cu_seqlens_kv_list[i] for i in range(len(cu_seqlens_kv_list) - 1)] if len(cu_seqlens_kv_list) > 1 else [0]
    ...
    _apply_golden_globals({
        ...
        "BLOCK_SIZE": block_size, "SPARSE_MODE": mask_mode,
        "Q_SCALE_LAYOUT": q_scale_layout, "P_SCALE": p_scale_val,
        "ENABLE_ROPE": False, "D_rope": 64,
        "ENABLE_LSE": enable_lse, "FP8_DTYPE": torch.float8_e4m3fn,
        "QUANT_GROUP_SIZE": 32, "INPUT_LAYOUT": input_layout,
