...
const aclTensor *TransposeView(const aclTensor *input, const std::vector<int64_t> &perm, aclOpExecutor *executor)
{
...
const aclTensor *TransposeLastTwoView(const aclTensor *input, aclOpExecutor *executor)
{
    ...
    return TransposeView(input, perm, executor);
}
...
static aclnnStatus ParseLayout(const char *layout, KdaFwdLayout &parsed)
{
    ...
    if (std::strcmp(layout, "BSND") == 0) {
        ...
        CHECK_COND(false, ACLNN_ERR_PARAM_INVALID, "layout must be uppercase and one of BSND, BNSD, TND or NTD.");
    }
    return ACLNN_SUCCESS;
}
...
aclnnStatus CheckCuSeqlens(const aclIntArray *cuSeqlens, int64_t seqlen)
{
    ...
    CHECK_COND((*cuSeqlens)[0] == 0, ACLNN_ERR_PARAM_INVALID, "cuSeqlensOptional[0] must be 0.");
...
aclnnStatus CheckChunkIndices(const aclIntArray *chunkIndices, const aclIntArray *cuSeqlens, int64_t totalChunks,
                              int64_t chunkSize)
{
    if (chunkIndices == nullptr) {
        return ACLNN_SUCCESS;
    }
    CHECK_COND(cuSeqlens != nullptr, ACLNN_ERR_PARAM_INVALID, "chunkIndicesOptional requires cuSeqlensOptional.");
    CHECK_COND(chunkIndices->Size() == static_cast<size_t>(totalChunks) * 2, ACLNN_ERR_PARAM_INVALID,
    ...
    for (size_t seq = 0; seq + 1 < cuSeqlens->Size(); ++seq) {
        ...
        for (int64_t chunk = 0; chunk < chunks; ++chunk) {
            CHECK_COND((*chunkIndices)[offset] == static_cast<int64_t>(seq) && (*chunkIndices)[offset + 1] == chunk,
                       ACLNN_ERR_PARAM_INVALID, "chunkIndicesOptional must use canonical sequence-major chunk order.");
            offset += 2;
        }
    }
    return ACLNN_SUCCESS;
...
aclnnStatus ResolveShapeInfo(const ChunkKdaFwdParams &params, KdaFwdLayout layout, KdaShapeInfo &info)
{
    ...
    CHECK_COND(Rank(params.q) == tensorRank && Rank(params.k) == tensorRank && Rank(params.v) == tensorRank &&
                   Rank(params.g) == tensorRank && Rank(params.beta) == betaRank,
               ACLNN_ERR_PARAM_INVALID,
               "q/k/v/g and beta ranks must match layout: rank3/rank2 for TND/NTD, rank4/rank3 for BSND/BNSD.");
    CHECK_COND(SameShape(params.q, params.k), ACLNN_ERR_PARAM_INVALID, "q and k must have identical shape.");
    ...
    if (layout == KdaFwdLayout::TND) {
        ...
        CHECK_COND(HasShape(params.v, {info.batch, info.seqlen, info.hvNum, info.vDim}) &&
                   ...
                   ACLNN_ERR_PARAM_INVALID, "BSND expects v/g/beta as [B,T,HV,V], [B,T,HV,K], [B,T,HV].");
    } else {
        info.batch = Dim(params.q, 0);
        info.hNum = Dim(params.q, 1);
        ...
        CHECK_COND(HasShape(params.v, {info.batch, info.hvNum, info.seqlen, info.vDim}) &&
                   ...
                   ACLNN_ERR_PARAM_INVALID, "BNSD expects v/g/beta as [B,HV,T,V], [B,HV,T,K], [B,HV,T].");
    }
    info.seqNum =
        params.cuSeqlensOptional == nullptr ? info.batch : static_cast<int64_t>(params.cuSeqlensOptional->Size()) - 1;
...
aclnnStatus CheckDtypes(const ChunkKdaFwdParams &params)
{
    ...
    CHECK_COND(gateType == DataType::DT_FLOAT || gateType == DataType::DT_BF16, ACLNN_ERR_PARAM_INVALID,
               "g must be float32 or bfloat16.");
    const DataType betaType = params.beta->GetDataType();
    CHECK_COND(betaType == DataType::DT_FLOAT || betaType == DataType::DT_BF16, ACLNN_ERR_PARAM_INVALID,
...
aclnnStatus CheckStateShape(const aclTensor *state, const char *name, const KdaShapeInfo &info, bool stateVFirst)
{
    ...
    const bool valid = stateVFirst ? HasShape(state, {info.seqNum, info.hvNum, info.vDim, info.kDim}) :
                                     HasShape(state, {info.seqNum, info.hvNum, info.kDim, info.vDim});
    CHECK_COND(valid, ACLNN_ERR_PARAM_INVALID, "%s must be [N,HV,K,V] when stateVFirst=false and [N,HV,V,K] otherwise.",
               name);
    return ACLNN_SUCCESS;
}
...
aclnnStatus CheckOutputShapes(const ChunkKdaFwdParams &params, const KdaShapeInfo &info)
{
    ...
    const bool attnShapeValid = info.isRank3 ?
                                    ...
                                    HasShape(params.attnOut, {info.batch, info.seqlen, info.hvNum, info.vDim});
    CHECK_COND(attnShapeValid && params.attnOut->GetDataType() == dataType, ACLNN_ERR_PARAM_INVALID,
               "attnOut must match q dtype and use fixed sequence-major TND/BSND layout.");
    if (params.gkOut != nullptr) {
        const bool valid = info.isRank3 ? HasShape(params.gkOut, {info.hvNum, info.seqlen, info.kDim}) :
                                          HasShape(params.gkOut, {info.batch, info.hvNum, info.seqlen, info.kDim});
        CHECK_COND(valid && params.gkOut->GetDataType() == DataType::DT_FLOAT, ACLNN_ERR_PARAM_INVALID,
                   "gkOut must be float32 in fixed head-major NTD/BNSD layout.");
    }
    const aclTensor *matrixOutputs[] = {params.aqkOut, params.akkOut};
    for (const aclTensor *output : matrixOutputs) {
        const bool valid = info.isRank3 ? HasShape(output, {info.hvNum, info.seqlen, params.chunkSize}) :
                                          HasShape(output, {info.batch, info.hvNum, info.seqlen, params.chunkSize});
    ...
    for (const aclTensor *output : kOutputs) {
        ...
        const bool valid = info.isRank3 ? HasShape(output, {info.hvNum, info.seqlen, info.kDim}) :
                                          HasShape(output, {info.batch, info.hvNum, info.seqlen, info.kDim});
        CHECK_COND(valid && output->GetDataType() == dataType, ACLNN_ERR_PARAM_INVALID,
    ...
    for (const aclTensor *output : vOutputs) {
        ...
        const bool valid = info.isRank3 ? HasShape(output, {info.hvNum, info.seqlen, info.vDim}) :
                                          HasShape(output, {info.batch, info.hvNum, info.seqlen, info.vDim});
        CHECK_COND(valid && output->GetDataType() == dataType, ACLNN_ERR_PARAM_INVALID,
    ...
    if (params.hOut != nullptr) {
        const bool valid =
            params.stateVFirst ?
                (info.isRank3 ?
                     HasShape(params.hOut, {info.totalChunks, info.hvNum, info.vDim, info.kDim}) :
                     HasShape(params.hOut, {info.batch, info.totalChunks, info.hvNum, info.vDim, info.kDim})) :
                (info.isRank3 ?
                     HasShape(params.hOut, {info.totalChunks, info.hvNum, info.kDim, info.vDim}) :
                     HasShape(params.hOut, {info.batch, info.totalChunks, info.hvNum, info.kDim, info.vDim}));
    ...
    CHECK_RET(CheckStateShape(params.finalStateOut, "finalStateOut", info, params.stateVFirst) == ACLNN_SUCCESS,
              ACLNN_ERR_PARAM_INVALID);
...
aclnnStatus CheckParams(const ChunkKdaFwdParams &params, KdaFwdLayout &layout, KdaShapeInfo &info)
{
    ...
    CHECK_COND(params.aqkOut != nullptr && params.akkOut != nullptr, ACLNN_ERR_PARAM_NULLPTR,
               "aqkOut and akkOut must not be nullptr.");
    ...
    CHECK_COND(info.hNum > 0 && info.hvNum >= info.hNum && info.hvNum % info.hNum == 0, ACLNN_ERR_PARAM_INVALID,
               "H and HV must be positive, HV must be greater than or equal to H, and HV must be divisible by H.");
    CHECK_COND(info.hNum <= MAX_KDA_HEAD_NUM && info.hvNum <= MAX_KDA_HEAD_NUM, ACLNN_ERR_PARAM_INVALID,
               "H and HV must be less than or equal to 128.");
    CHECK_COND(info.kDim >= 16 && info.kDim <= MAX_KDA_K_DIM && info.kDim % 16 == 0 && info.vDim >= 16 &&
                   info.vDim <= 256 && info.vDim % 16 == 0,
               ACLNN_ERR_PARAM_INVALID, "K/V must be multiples of 16, K must be <=256, and V must be <=256.");
    CHECK_RET(CheckDtypes(params) == ACLNN_SUCCESS, ACLNN_ERR_PARAM_INVALID);
    CHECK_RET(CheckCuSeqlens(params.cuSeqlensOptional, info.seqlen) == ACLNN_SUCCESS, ACLNN_ERR_PARAM_INVALID);
    CHECK_COND(params.cuSeqlensOptional == nullptr || info.isRank3 || info.batch == 1, ACLNN_ERR_PARAM_INVALID,
               "rank4 varlen input with cuSeqlensOptional requires B=1.");
    CHECK_COND(params.cuSeqlensOptional == nullptr || info.seqNum <= MAX_KDA_VARLEN_SEQUENCES, ACLNN_ERR_PARAM_INVALID,
               "varlen input supports at most 1024 sequences.");
    CHECK_RET(CheckChunkIndices(params.chunkIndicesOptional, params.cuSeqlensOptional, info.totalChunks,
                                params.chunkSize) == ACLNN_SUCCESS,
              ACLNN_ERR_PARAM_INVALID);
    CHECK_RET(
        CheckStateShape(params.initialStateOptional, "initialStateOptional", info, params.stateVFirst) == ACLNN_SUCCESS,
        ACLNN_ERR_PARAM_INVALID);
    if (params.useGateInKernel) {
        ...
        if (params.dtBiasOptional != nullptr) {
            CHECK_COND(HasShape(params.dtBiasOptional, {info.hvNum * info.kDim}), ACLNN_ERR_PARAM_INVALID,
                       "dtBiasOptional must have shape [HV*K].");
        }
        if (params.safeGate) {
            CHECK_COND(params.lowerBound >= -5.0 && params.lowerBound < 0.0, ACLNN_ERR_PARAM_INVALID,
                       "lowerBound must be in [-5, 0) when safeGate and useGateInKernel are true.");
        }
    }
...
aclnnStatus ContiguousInputs(ChunkKdaFwdParams &params, aclOpExecutor *executor)
{
    ...
    CHECK_RET(MakeContiguous(params.initialStateOptional, executor) == ACLNN_SUCCESS, ACLNN_ERR_PARAM_INVALID);
    return ACLNN_SUCCESS;
}
...
aclnnStatus aclnnChunkKdaFwdGetWorkspaceSize(const aclTensor *q, const aclTensor *k, const aclTensor *v,
                                             const aclTensor *g, const aclTensor *beta, const aclTensor *aLogOptional,
                                             ...
                                             bool stateVFirst, const aclTensor *attnOut, const aclTensor *finalStateOut,
                                             const aclTensor *gkOut, const aclTensor *aqkOut, const aclTensor *akkOut,
                                             const aclTensor *wOut, const aclTensor *uOut, const aclTensor *qgOut,
                                             const aclTensor *kgOut, const aclTensor *vNewOut, const aclTensor *hOut,
                                             uint64_t *workspaceSize, aclOpExecutor **executor)
{
    ChunkKdaFwdParams params{q,
                             k,
                             v,
                             g,
                             beta,
    ...
    L2_DFX_PHASE_1(
        ...
        DFX_IN(q, k, v, g, beta, aLogOptional, dtBiasOptional, initialStateOptional, cuSeqlensOptional,
               chunkIndicesOptional, layout, scale, chunkSize, safeGate, lowerBound, useGateInKernel, stateVFirst),
        DFX_OUT(attnOut, finalStateOut, gkOut, aqkOut, akkOut, wOut, uOut, qgOut, kgOut, vNewOut, hOut));
    ...
    if (parsedLayout == KdaFwdLayout::BSND) {
        ...
        betaHead = TransposeView(params.beta, {0, 2, 1}, executorPtr);
    } else if (parsedLayout == KdaFwdLayout::TND) {
        qHead = TransposeView(params.q, {1, 0, 2}, executorPtr);
        kHead = TransposeView(params.k, {1, 0, 2}, executorPtr);
        vHead = TransposeView(params.v, {1, 0, 2}, executorPtr);
        gHead = TransposeView(params.g, {1, 0, 2}, executorPtr);
        betaHead = TransposeView(params.beta, {1, 0}, executorPtr);
    }
    CHECK_RET(qHead != nullptr && kHead != nullptr && vHead != nullptr && gHead != nullptr && betaHead != nullptr,
    ...
    const op::Shape matrixShape4 = MakeShape({info.batch, info.hvNum, info.seqlen, params.chunkSize});
    ...
    const op::Shape hShape5 = MakeShape({info.batch, info.hvNum, info.totalChunks, info.kDim, info.vDim});
    const op::Shape hExportShape5 = params.stateVFirst ?
    ...
    const op::Shape stateShape4 = MakeShape({info.seqNum, info.hvNum, info.kDim, info.vDim});
    ...
    CHECK_RET(
        (params.wOut == nullptr || wExport != nullptr) && (params.uOut == nullptr || uExport != nullptr) &&
            (params.qgOut == nullptr || qgExport != nullptr) && (params.kgOut == nullptr || kgExport != nullptr) &&
            (params.vNewOut == nullptr || vNewExport != nullptr) && (params.hOut == nullptr || hExport != nullptr),
        ACLNN_ERR_INNER_NULLPTR);
    ...
    const aclTensor *wCompute =
        wExport == nullptr ? AllocTensor(executorPtr, placeholderShape, params.q->GetDataType()) : wExport;
    const aclTensor *uCompute =
        uExport == nullptr ? AllocTensor(executorPtr, placeholderShape, params.q->GetDataType()) : uExport;
    const aclTensor *qgCompute =
        qgExport == nullptr ? AllocTensor(executorPtr, placeholderShape, params.q->GetDataType()) : qgExport;
    const aclTensor *kgCompute =
        kgExport == nullptr ? AllocTensor(executorPtr, placeholderShape, params.q->GetDataType()) : kgExport;
    const aclTensor *vNewCompute =
        vNewExport == nullptr ? AllocTensor(executorPtr, placeholderShape, params.q->GetDataType()) : vNewExport;
    const aclTensor *hCompute =
        AllocTensor(executorPtr, hExport == nullptr ? placeholderShape : hShape5, params.q->GetDataType());
    CHECK_RET(aqkCompute != nullptr && akkCompute != nullptr && wCompute != nullptr && uCompute != nullptr &&
                  qgCompute != nullptr && kgCompute != nullptr && vNewCompute != nullptr && hCompute != nullptr,
              ACLNN_ERR_INNER_NULLPTR);

    const aclTensor *initialStateCompute = params.initialStateOptional;
    if (params.stateVFirst && initialStateCompute != nullptr) {
        initialStateCompute = TransposeLastTwoView(initialStateCompute, executorPtr);
        CHECK_RET(initialStateCompute != nullptr, ACLNN_ERR_INNER_NULLPTR);
    }
    const bool outputFinalState = params.finalStateOut != nullptr;
    const aclTensor *finalStateCompute =
        AllocTensor(executorPtr, outputFinalState ? stateShape4 : placeholderShape, DataType::DT_FLOAT);
    CHECK_RET(finalStateCompute != nullptr, ACLNN_ERR_INNER_NULLPTR);

    const aclTensor *attnCompute = params.attnOut;
    if (info.isRank3) {
        attnCompute = AsRank4(params.attnOut, MakeShape({1, info.seqlen, info.hvNum, info.vDim}), executorPtr);
        CHECK_RET(attnCompute != nullptr, ACLNN_ERR_INNER_NULLPTR);
    }

    auto result = l0op::KdaChunkForward(qHead, kHead, vHead, gHead, betaHead, params.aLogOptional,
                                        ...
                                        parsedLayout == KdaFwdLayout::BSND, params.useGateInKernel, params.lowerBound,
                                        attnCompute, finalStateCompute, gkCompute, aqkCompute, akkCompute, wCompute,
                                        uCompute, qgCompute, kgCompute, vNewCompute, hCompute, executorPtr);
    ...
    if (outputFinalState) {
        ...
        if (params.stateVFirst) {
            finalStateResult = TransposeLastTwoView(finalStateResult, executorPtr);
    ...
    if (hExport != nullptr) {
        const std::vector<int64_t> hPerm =
            params.stateVFirst ? std::vector<int64_t>{0, 2, 1, 4, 3} : std::vector<int64_t>{0, 2, 1, 3, 4};
        const aclTensor *hResult = TransposeView(result[10], hPerm, executorPtr);
        CHECK_RET(hResult != nullptr, ACLNN_ERR_INNER_NULLPTR);
        CHECK_RET(l0op::ViewCopy(hResult, hExport, executorPtr) != nullptr, ACLNN_ERR_INNER_NULLPTR);
    }
