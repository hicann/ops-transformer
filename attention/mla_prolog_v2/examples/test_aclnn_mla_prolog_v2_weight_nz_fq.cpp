...
/*!
 * \file test_aclnn_mla_prolog_v2_weight_nz_fq.cpp
...
int main()
{
    ...
    ret = CreateAclTensorNZ(weightDkvKrShape, &weightDkvKrDeviceAddr, &weightDkvKrHostAddr, aclDataType::ACL_INT8,
                            &weightDkvKr);
    CHECK_RET(ret == ACL_SUCCESS, return ret);
    // 创建ropeSin aclTensor
    ...
    ret = CreateAclTensorND(dequantScaleWDkvKrShape, &dequantScaleWDkvKrDeviceAddr, &dequantScaleWDkvKrHostAddr,
                            aclDataType::ACL_FLOAT, &dequantScaleWDkvKr);
    CHECK_RET(ret == ACL_SUCCESS, return ret);
    ...
    ret = aclnnMlaPrologV2WeightNzGetWorkspaceSize(
        ...
        quantScaleCkv, nullptr, smoothScalesCq, rmsnormEpsilonCq, rmsnormEpsilonCkv, cacheMode, query, queryRope,
        dequantScaleQNope, &workspaceSize, &executor);
