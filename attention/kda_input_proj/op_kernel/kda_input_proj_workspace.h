...
struct KdaInputProjWorkspace {
    ...
    static inline uint64_t CeilAlign(uint64_t value, uint64_t align)
    {
        return align == 0UL ? value : CeilDiv(value, align) * align;
    }

    // Matches Blaze MX scaleKLen: CeilDiv(K, 64) * 2.
    static inline uint64_t ScaleKLen(uint32_t hiddenSize)
    {
        return CeilDiv(static_cast<uint64_t>(hiddenSize), MXFP_GROUP_K) * MXFP_SCALE_MULTI;
    }

    static inline uint64_t QuantXBytes(uint32_t tSize, uint32_t hiddenSize)
    {
        return static_cast<uint64_t>(tSize) * static_cast<uint64_t>(hiddenSize);
    }

    static inline uint64_t XScaleBytes(uint32_t tSize, uint32_t hiddenSize)
    {
        return static_cast<uint64_t>(tSize) * ScaleKLen(hiddenSize);
    }

    static inline uint64_t OffsetQuantX()
    {
        return 0UL;
    }

    static inline uint64_t OffsetScaleX(uint32_t tSize, uint32_t hiddenSize)
    {
        return CeilAlign(QuantXBytes(tSize, hiddenSize), ALIGN);
    }

    static inline uint64_t TotalBytes(uint32_t tSize, uint32_t hiddenSize)
    {
