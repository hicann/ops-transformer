...
def _normalize_legacy_pt_data(test_data):
    ...
    tensors = [test_data["query"], test_data["key"]]
    if test_data.get("blockFusion") is not None:
        tensors.append(test_data["blockFusion"])
    if tensors[0].dtype not in (torch.float16, torch.bfloat16) or any(
        tensor.dtype != tensors[0].dtype for tensor in tensors[1:]
    ):
        raise ValueError(
            "LI_V2 PT has inconsistent Q/K/blockFusion dtypes; regenerate the PT "
            "instead of converting inputs independently of the saved CPU golden"
        )
    if params[10] not in ("FP16", "BF16", torch.float16, torch.bfloat16) or (
        params[10] in ("BF16", torch.bfloat16) and tensors[0].dtype != torch.bfloat16
    ):
        raise ValueError(
            "LI_V2 PT has an unsupported dtype mismatch; regenerate the PT"
        )
    # Older generators interpreted torch.float16 parameters as BF16.
    if params[10] in ("FP16", torch.float16) and all(
        tensor.dtype == torch.bfloat16 for tensor in tensors
    ):
        params = list(params)
        params[10] = torch.bfloat16
        test_data["params"] = tuple(params)
        logging.warning(
            "Legacy LI_V2 PT declares FP16 but stores BF16; replaying stored BF16 inputs"
        )
    if test_data["layout_query"] == "BSND":
        ...
        if test_data["max_seqlen_q_meta"] != max_seqlen_q_meta:
            logging.warning(
                "Legacy LI_V2 PT metadata query length corrected from %s to %s",
...
def test_liv2_process(filepath, device_id=0):
    ...
    if params[10] == "FP16" or params[10] == torch.float16:
    ...
    elif params[10] == "BF16" or params[10] == torch.bfloat16:
        if "blockFusion" in test_data and test_data["blockFusion"] is not None:
