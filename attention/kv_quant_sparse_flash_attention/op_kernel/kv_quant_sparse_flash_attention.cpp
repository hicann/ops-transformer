...
#define QSFA_OP_IMPL(templateClass, tilingdataClass, ...) \
    do { \
        ...
        op.Init(query, key, value, sparseIndices, keyScale, valueScale, blocktable, actualSeqLengthsQuery, \
                actualSeqLengthsKV, sinks, attentionOut, user, nullptr, &tPipe); \
        op.Process(); \
    } while (0)
#else
#define QSFA_OP_IMPL(templateClass, tilingdataClass, ...) \
    do { \
        ...
        op.Init(query, key, value, sparseIndices, keyScale, valueScale, blocktable, actualSeqLengthsQuery, \
                actualSeqLengthsKV, sinks, attentionOut, user, tilingData, &tPipe); \
        op.Process(); \
    } while (0)
#endif
#else
#define QSFA_OP_IMPL(templateClass, tilingdataClass, ...) \
    do { \
        ...
        op.Init(query, key, value, sparseIndices, keyScale, valueScale, blocktable, actualSeqLengthsQuery, \
                actualSeqLengthsKV, attentionOut, user, tiling_data, tiling, &tPipe); \
        op.Process(); \
    } while (0)
#endif
...
__aicore__ inline void DispatchKernelDtype310(__gm__ uint8_t *query, __gm__ uint8_t *key, __gm__ uint8_t *value,
                                              ...
                                              __gm__ uint8_t *sinks, __gm__ uint8_t *attentionOut, __gm__ uint8_t *user,
    ...
    __gm__ uint8_t *attentionOut, __gm__ uint8_t *workspace, __gm__ uint8_t *tiling)
{
    ...
    if constexpr (ORIG_DTYPE_QUERY == DT_FLOAT16 && ORIG_DTYPE_KEY == DT_INT8 &&
                  ORIG_DTYPE_ATTENTION_OUT == DT_FLOAT16) {
        QSFA_OP_IMPL(KvQuantSparseFlashAttentionMla, KvQuantSparseFlashAttentionTilingDataMla, half, int8_t, half,
                     FLASH_DECODE, static_cast<QSFA_LAYOUT>(LAYOUT_T), static_cast<QSFA_LAYOUT>(KV_LAYOUT_T),
                     TEMPLATE_MODE);
    } else { // bf16
        QSFA_OP_IMPL(KvQuantSparseFlashAttentionMla, KvQuantSparseFlashAttentionTilingDataMla, bfloat16_t, int8_t,
                     bfloat16_t, FLASH_DECODE, static_cast<QSFA_LAYOUT>(LAYOUT_T),
