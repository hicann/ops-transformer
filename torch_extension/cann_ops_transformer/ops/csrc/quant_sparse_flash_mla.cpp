...
constexpr int64_t QSMLA_METADATA_SIZE = 1024;

inline TensorWrapper QsmlaMakeWrapper(const at::Tensor &tensor, aclDataType tensorAcltype)
{
    return {tensor, tensorAcltype};
}

at::Tensor QuantSparseFlashMlaMetadata(
    ...
    c10::string_view layoutQ, c10::string_view layoutKv, bool hasOriKv, bool hasCmpKv)
{
    ...
    at::Tensor output = torch::empty({QSMLA_METADATA_SIZE}, torch::dtype(torch::kInt32).device(outputDevice));
    ...
    bool returnSoftmaxLse)
{
    ...
    std::tuple<at::Tensor, at::Tensor> quantSparseFlashMlaAttenOut =
