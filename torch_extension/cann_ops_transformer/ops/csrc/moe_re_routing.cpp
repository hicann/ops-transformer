...
std::tuple<at::Tensor, at::Tensor, at::Tensor, at::Tensor, at::Tensor> moe_re_routing(
    ...
    const c10::optional<at::Tensor> &expert_topk_weight_opt,
    ...
    c10::optional<int64_t> tokens_dtype)
{
    ...
    if (tokens_dtype.has_value()) {
        ...
        TORCH_CHECK(td == static_cast<int64_t>(DType::HIFLOAT8) ||
                        td == static_cast<int64_t>(DType::FLOAT4_E2M1) ||
...

    {
        ...
        if (has_topk_weight) {
            permute_topk_weight = at::empty(expert_topk_weight_opt.value().sizes(),
                                            expert_topk_weight_opt.value().options());
    ...
    aclDataType tokens_acl_dtype = tokens_dtype.has_value() ? GetAclDataType(tokens_dtype.value())
                                                             : ConvertToAclDataType(tokens.scalar_type());
    TensorWrapper tokens_wrapper = {tokens, tokens_acl_dtype};
    aclDataType permute_tokens_acl_dtype =
        tokens_dtype.has_value() ? GetAclDataType(tokens_dtype.value())
    ...
    if (per_token_scales_dtype == at::kByte) {
        ...
        ACLNN_CMD(aclnnMoeReRoutingV2, tokens_wrapper, expert_token_num_per_rank, per_token_scales_wrapper,
                  expert_topk_weight_opt, expert_token_num_type, idx_type,
                  permute_tokens_wrapper, permute_per_token_scales_wrapper,
                  permute_token_idx, expert_token_num, permute_topk_weight_opt);
    } else {
        ACLNN_CMD(aclnnMoeReRoutingV2, tokens_wrapper, expert_token_num_per_rank, per_token_scales,
                  expert_topk_weight_opt, expert_token_num_type, idx_type,
                  permute_tokens_wrapper, permute_per_token_scales,
                  permute_token_idx, expert_token_num, permute_topk_weight_opt);
    }
...
{
    m.def("moe_re_routing", &moe_re_routing, "moe_re_routing");
}
