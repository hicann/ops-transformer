...
#include "nn_other.h"
#include "../op_graph/moe_re_routing_v2_proto.h"
...
#define ADD_INPUT(intputIndex, intputName, intputDtype, inputShape) \
    ...
    moe_re_routing_v2_op.set_input_##intputName(placeholder##intputIndex); \
    inputs.push_back(placeholder##intputIndex);

#define ADD_INPUT_ATTR(attrName, attrValue) moe_re_routing_v2_op.set_attr_##attrName(attrValue);
...
#define ADD_CONST_INPUT(intputIndex, intputName, intputDtype, inputShape) \
    ...
    graph.AddOp(placeholder##intputIndex); \
    moe_re_routing_v2_op.set_input_##intputName(placeholder##intputIndex); \
    moe_re_routing_v2_op.update_input_desc_##intputName(placeholder##intputIndex##_desc); \
    inputs.push_back(placeholder##intputIndex);

#define ADD_OUTPUT(outputIndex, outputName, outputDtype, outputShape) \
    TensorDesc outputName##outputIndex##_desc = TensorDesc(ge::Shape(outputShape), FORMAT_ND, outputDtype); \
    moe_re_routing_v2_op.update_output_desc_##outputName(outputName##outputIndex##_desc);
                     ...
                     std::vector<Operator> &outputs, Graph &graph)
{
    Status ret = SUCCESS;
    // 自定义代码：添加单算子定义到图中
    auto moe_re_routing_v2_op = op::MoeReRoutingV2("test_geir_moe_re_routing_v2");

    // shape定义
    std::vector<int64_t> tokens_shape = {A, H};
    std::vector<int64_t> num_in_shape = {N, E};
    std::vector<int64_t> scales_in_shape = {A, S};
    std::vector<int64_t> topk_weight_in_shape = {A, 1};
    ...
    std::vector<int64_t> topk_weight_out_shape = {A, 1};
    ...
    ADD_INPUT(4, expert_topk_weight, DT_FLOAT, topk_weight_in_shape);

    // 添加属性
    ADD_INPUT_ATTR(expert_token_num_type, 1);
    ADD_INPUT_ATTR(idx_type, 0);
    ...
    ADD_OUTPUT(5, permute_topk_weight, DT_FLOAT, topk_weight_out_shape);

    outputs.push_back(moe_re_routing_v2_op);
    // 添加完毕
    return SUCCESS;
}
...
int main()
{
    const char *graph_name = "tc_geir_test_moe_re_routing_v2";
    ...
    for (int i = 0; i < input_num; i++) {
        ...
        string input_file = "./tc_geir_test_moe_re_routing_v2_npu_input_" + std::to_string(i) + ".bin";
    ...
    for (int i = 0; i < output_num; i++) {
        ...
        string output_file = "./tc_geir_test_moe_re_routing_v2_npu_output_" + std::to_string(i) + ".bin";
