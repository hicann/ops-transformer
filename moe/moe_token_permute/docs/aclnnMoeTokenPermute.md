...
<!-- npu="950" id7 -->
- <term>Ascend 950PR/Ascend 950DT</term>：仅该产品支持INT8类型的`tokens`和`permuteTokensOut`，按非量化方式处理，且二者数据类型必须一致；INT8时`tokens`维度必须为2，不支持3维及以上。
<!-- end id7 -->
<!-- npu="950" id8 -->
- <term>Ascend 950PR/Ascend 950DT</term>：当`tokens`数据类型为INT8时，`indices`表示expert ID，取值范围为`[0, 10240)`，最大值为`10239`，不支持`10240`。
<!-- end id8 -->
