...
#include "../op_graph/moe_init_routing_v4_proto.h"
#include "nn_other.h"
...
#define ADD_INPUT(intputIndex, intputName, intputDtype, inputShape) \
    ...
    MIRV4op.set_input_##intputName(placeholder##intputIndex); \
    inputs.push_back(placeholder##intputIndex);

#define ADD_INPUT_ATTR(attrName, attrValue) MIRV4op.set_attr_##attrName(attrValue);

#define ADD_OUTPUT(outputIndex, outputName, outputDtype, outputShape) \
    TensorDesc outputName##outputIndex##_desc = TensorDesc(ge::Shape(outputShape), FORMAT_ND, outputDtype); \
    MIRV4op.update_output_desc_##outputName(outputName##outputIndex##_desc);
                     ...
                     std::vector<Operator> &outputs, Graph &graph)
{
    Status ret = SUCCESS;
    // 自定义代码：添加单算子定义到图中
    auto MIRV4op = op::MoeInitRoutingV4("test_geir_moe_init_routing_v4");
    ...
    std::vector<int64_t> active_num_shape = {};
    std::vector<int64_t> topk_weight_shape = {B * S, K};
    ...
    std::vector<int64_t> expanded_topk_weight_shape = {B * S * K, 1};
    ...
    ADD_INPUT(5, active_num, DT_INT64, active_num_shape);
    ADD_INPUT(6, topk_weight, DT_FLOAT, topk_weight_shape);
    ...
    ADD_OUTPUT(5, expanded_topk_weight, DT_FLOAT, expanded_topk_weight_shape);
