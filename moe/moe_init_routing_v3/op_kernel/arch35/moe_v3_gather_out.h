                                             ...
                                             const MoeInitRoutingV3Arch35TilingData *tilingData, TPipe *tPipe)
{
    ...
    pipe_->InitBuffer(xCopyInQueue_, xCopyInQueueBufferNum,
                      curCorePerLoopIndicesElements_ * AlignBytes(perLoopCols_, sizeof(T)));
...
__aicore__ inline void MoeGatherOut<T>::Process()
{
    if (blockIdx_ < needCoreNum_) {
        ...
        for (int64_t indicesLoop = 0; indicesLoop < indicesLoops_; indicesLoop++) {
            ...
            if (colsLoops_ == 1) {
                CopyXOut(curExpertLoopOffset * cols_, curLoopElements, cols_, Align(perLoopCols_, sizeof(T)));
            }
        }
    }
}
