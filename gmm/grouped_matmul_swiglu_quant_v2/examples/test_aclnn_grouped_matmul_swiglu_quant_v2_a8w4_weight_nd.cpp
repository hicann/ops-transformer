...
int main()
{
    ...
    ret = CreateAclTensorList(weightHostData, weightShape, weightDeviceAddr, aclDataType::ACL_INT32,
                              aclFormat::ACL_FORMAT_ND, &weight);
    CHECK_RET(ret == ACL_SUCCESS, return ret);
    // 创建weightScale aclTensorList
    ret = CreateAclTensorList(weightScaleHostData, weightScaleShape, weightScaleDeviceAddr, aclDataType::ACL_UINT64,
                              aclFormat::ACL_FORMAT_ND, &weightScale);
    CHECK_RET(ret == ACL_SUCCESS, return ret);
    // 创建weightAssistMatrix aclTensorList
    ret = CreateAclTensorList(weightAssistMatrixHostData, weightAssistMatrixShape, weightAssistMatrixDeviceAddr,
                              aclDataType::ACL_FLOAT, aclFormat::ACL_FORMAT_ND, &weightAssistMatrix);
    CHECK_RET(ret == ACL_SUCCESS, return ret);
    // 创建xScale aclTensor
