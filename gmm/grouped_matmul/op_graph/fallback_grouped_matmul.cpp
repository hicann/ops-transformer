...
static inline aclDataType ToAclDataTypeForGMM(ge::DataType dtype)
{
    static const std::vector<DataType> GMM_CONVERT_TO_ACL_DataType_LIST = {
        ...
        ge::DataType::DT_FLOAT8_E8M0, ge::DataType::DT_FLOAT4_E2M1};
...
static bool SetShapeAndStrideForLastTwoDimForGMM(std::vector<int64_t> &viewShape, std::vector<int64_t> &strides)
{
    OP_CHECK_IF(
        viewShape.size() < 2,
        OP_LOGE_FOR_INVALID_SHAPEDIM_WITH_REASON("GroupedMatmul aclnnfallback", "viewShape",
                                                 std::to_string(viewShape.size()), "The dim num must be at least 2"),
        return false);
...
static inline aclTensor *GeTensor2AclTensor(const gert::Tensor *geTensor, bool enableTranspose, size_t index,
                                            bool enableNZ = false)
{
    OP_CHECK_IF(geTensor == nullptr,
                OP_LOGE_WITH_INVALID_INPUT("GroupedMatmul aclnnfallback",
                                           ("input tensor at index " + std::to_string(index)).c_str()),
                return nullptr);
    ...
    OP_CHECK_IF(aclCreateTensor == nullptr,
                OP_LOGE_FOR_INVALID_ARGUMENT_WITH_REASON("GroupedMatmul aclnnfallback", "aclCreateTensor",
                                                         "Failed to load opapi func aclCreateTensor"),
                return nullptr);
    ...
    if (enableTranspose) {
        if (dataTypeGE == ge::DataType::DT_FLOAT8_E8M0) {
            ...
            OP_CHECK_IF(!SetShapeAndStrideForLastTwoDimForGMM(viewShape, strides),
                        OP_LOGE_FOR_INVALID_ARGUMENT_WITH_REASON("GroupedMatmul aclnnfallback", "tensor",
                                                                 "SetShapeAndStrideForLastTwoDimForGMM failed"),
                        return nullptr);
        }
    }
    ...
    OP_CHECK_IF(out == nullptr,
                OP_LOGE_FOR_INVALID_ARGUMENT_WITH_REASON("GroupedMatmul aclnnfallback", "output tensor",
                                                         "aclCreateTensor failed to create an ACL tensor"),
                return nullptr);
    return out;
}
                                             ...
                                             size_t numGeWeight, int32_t splitItem)
{
    size_t numGeY = 0;
    if (0 == splitItem ||
        1 == splitItem) { // Length of tensorListY equals that of tensorListWeight when split_item = 0 / 1
        numGeY = numGeWeight;
    } else if (2 == splitItem || 3 == splitItem) { // Length of tensorListY equals 1 when split_item = 2 / 3
        numGeY = 1;
    } else {
        OP_LOGE_FOR_INVALID_VALUE("GroupedMatmul aclnnfallback", "split_item", std::to_string(splitItem), "0/1/2/3");
        return GRAPH_FAILED;
    }
...
static bool IsPerTileQuantMode(const OpExecuteContext *host_api_ctx, bool transposeX, bool transposeWeight,
                               int64_t groupType)
{
    ...
    if (x == nullptr || weight == nullptr || scale == nullptr || perTokenScaleTensor == nullptr ||
        groupListTensor == nullptr) {
        return false;
    }
    ...
    auto perTokenMDim = transposeX ? perTokenScaleOriginShape.GetDim(xDimNum - 1) :
                                     perTokenScaleOriginShape.GetDim(xDimNum - PENULTIMATE_DIM);
    ...
    auto scaleKDim =
        transposeWeight ? scaleShape.GetDim(scaleDimNum - 1) : scaleShape.GetDim(scaleDimNum - PENULTIMATE_DIM);
    auto scaleNDim =
        transposeWeight ? scaleShape.GetDim(scaleDimNum - PENULTIMATE_DIM) : scaleShape.GetDim(scaleDimNum - 1);
    auto weightKDim =
        transposeWeight ? weightShape.GetDim(weightDimNum - 1) : weightShape.GetDim(weightDimNum - PENULTIMATE_DIM);
    auto weightNDim =
        transposeWeight ? weightShape.GetDim(weightDimNum - PENULTIMATE_DIM) : weightShape.GetDim(weightDimNum - 1);
    ...
    if (perTokenScale == nullptr) {
        ...
        OP_CHECK_IF(aclCreateTensor == nullptr,
                    OP_LOGE_FOR_INVALID_ARGUMENT_WITH_REASON("GroupedMatmul aclnnfallback", "aclCreateTensor",
                                                             "Failed to load opapi func aclCreateTensor"),
                    return GRAPH_FAILED);
        perTokenScale = aclCreateTensor(shape.data(), shape.size(), aclDataType::ACL_FLOAT, shape.data(), 0,
                                        aclFormat::ACL_FORMAT_ND, shape.data(), shape.size(), nullptr);
        OP_CHECK_IF(
            perTokenScale == nullptr,
            OP_LOGE_FOR_INVALID_ARGUMENT_WITH_REASON("GroupedMatmul aclnnfallback", "perTokenScale",
                                                     "aclCreateTensor failed to create the perTokenScale placeholder"),
            return GRAPH_FAILED);
    }
