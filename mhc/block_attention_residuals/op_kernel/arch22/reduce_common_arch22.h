 ...
 */
/*!
 * \file reduce_common_arch22.h
 * \brief Half-interval reduce; vecMeta helpers — **禁止** LocalTensor::GetValue/SetValue。
 */
#ifndef REDUCE_COMMON_ARCH22_H
#define REDUCE_COMMON_ARCH22_H
 ...
 * NOTE: src is destroyed in-place. No GetValue/SetValue.
 * 支持 count > ELEM_PER_REP_FP32（64）。尾区 fold 需 src 容量 ≥ RoundUpFp32(count)。
 */
...
/*!
 * UB→UB 紧凑 16bit（bf16/half）行拷贝：Vector Copy（PIPE_V）。
 * 16bit 类型单次 mask∈[1,128]（256B/rep，与 fp32 64 元素等价，见 AscendCUtils::SetMask）；对任意
 * elemCount 按 128 切段（repeat≤255），支持 >128/>512。dst/src 基址须 32B 对齐。
 */
template <typename T>
__aicore__ inline void CopyCompactHalfsUb(const LocalTensor<T> &dst, const LocalTensor<T> &src, uint32_t elemCount)
{
    if (elemCount == 0) {
        return;
    }
    constexpr uint32_t ELEM_PER_REP_HALF = ELEM_PER_REP_FP32 * 2U; // 128
    uint32_t offset = 0;
    uint32_t remain = elemCount;
    while (remain >= ELEM_PER_REP_HALF) {
        const uint32_t reps = (remain / ELEM_PER_REP_HALF > MAX_REP_NUM) ? MAX_REP_NUM : (remain / ELEM_PER_REP_HALF);
        Copy(dst[offset], src[offset], static_cast<uint64_t>(ELEM_PER_REP_HALF), static_cast<uint8_t>(reps),
             {1, 1, 8, 8});
        offset += reps * ELEM_PER_REP_HALF;
        remain -= reps * ELEM_PER_REP_HALF;
    }
    if (remain > 0) {
        Copy(dst[offset], src[offset], static_cast<uint64_t>(remain), 1, {1, 1, 8, 8});
    }
                                        ...
                                        uint32_t hiddenSizeAlignFp32)
{
    ...
    const BinaryRepeatParams repeatParams{
        1, 1, 0, static_cast<uint8_t>(MUL_BRC_REP_STRIDE), static_cast<uint8_t>(MUL_BRC_REP_STRIDE), 0};
    SetMaskCount();
 ...
 * FP32 输入：同型直拷（Copy Level0，isSetMask=false，Counter 外置）。
 */
...
__aicore__ inline void CastRowToFp32CounterNoSetMask(const LocalTensor<float> &dst, const LocalTensor<SrcT> &src)
{
    if constexpr (IsSameType<SrcT, float>::value) {
        // fp32→fp32：与 16bit→fp32 等价步长（dstRep=8，src 每 repeat 64 元素）
        const CopyRepeatParams copyParams{1, 1, static_cast<uint8_t>(MUL_BRC_REP_STRIDE),
                                          static_cast<uint8_t>(MUL_BRC_REP_STRIDE)};
        Copy<float, false>(dst, src, MASK_PLACEHOLDER, 1, copyParams);
    } else {
...
/*! dst(fp32) = src：16bit Cast 或 fp32 直拷（普通 mask，非 Counter）。 */
template <typename SrcT>
__aicore__ inline void CastRowToFp32(const LocalTensor<float> &dst, const LocalTensor<SrcT> &src, uint32_t elemCount)
{
    if constexpr (IsSameType<SrcT, float>::value) {
        CopyCompactFloatsUb(dst, src, elemCount);
    } else {
        Cast(dst, src, RoundMode::CAST_NONE, elemCount);
        PipeBarrier<PIPE_V>();
    }
}

/*! dst = fp32 src：16bit 输出 CAST_RINT 或 fp32 直拷。 */
template <typename DstT>
__aicore__ inline void CastFp32ToRow(const LocalTensor<DstT> &dst, const LocalTensor<float> &src, uint32_t elemCount)
{
    if constexpr (IsSameType<DstT, float>::value) {
        CopyCompactFloatsUb(dst, src, elemCount);
    } else {
        Cast(dst, src, RoundMode::CAST_RINT, elemCount);
        PipeBarrier<PIPE_V>();
    }
}

/*! UB→UB 紧凑拷贝（16bit 用 128 元素/rep，fp32 用 64 元素/rep）。 */
template <typename T>
__aicore__ inline void CopyCompactUb(const LocalTensor<T> &dst, const LocalTensor<T> &src, uint32_t elemCount)
{
    if constexpr (sizeof(T) == 2) {
        CopyCompactHalfsUb(dst, src, elemCount);
    } else {
        CopyCompactFloatsUb(dst, src, elemCount);
    }
}
                                           ...
                                           const LocalTensor<float> &tmpBuffer, int32_t curColNum)
{
    ...
    if (numRemainPerLine > 0) {
        ...
        Mul(output[numRepeatPerLine * static_cast<int32_t>(ELEM_PER_REP_FP32)],
            input0[numRepeatPerLine * static_cast<int32_t>(ELEM_PER_REP_FP32)], tmpBuffer, remAlign, 1, instrParams);
        PipeBarrier<PIPE_V>();
    }
}
 ...
 * 支持 blockCount > 64（N≤100，blockCount=2..101）：
 *   - max/sum 归约在独立 foldWork 上进行（HalfInterval fold），不破坏 brcMeta 的 score/exp 值。
 * 容量要求：brcPack ≥ metaAlign + ELEM_PER_BLK_FP32 float（brcScratch + foldWork）。
 */
                                       ...
                                       const LocalTensor<float> &brcPack)
{
    ...
    const LocalTensor<float> foldWork = brcPack[metaAlign];
    const int32_t curColNum = static_cast<int32_t>(blockCount);

    // brcMeta = score（pad 区 -1e20，供 max fold / Sub / Exp）
    Duplicate(brcMeta, SOFTMAX_PAD, metaAlign);
    PipeBarrier<PIPE_V>();
    CopyCompactFloatsUb(brcMeta, vecMeta, blockCount);

    // max 归约：在 foldWork 上进行（破坏 foldWork，保留 brcMeta 的原始 score）
    Duplicate(foldWork, SOFTMAX_PAD, metaAlign);
    PipeBarrier<PIPE_V>();
    CopyCompactFloatsUb(foldWork, vecMeta, blockCount);
    ReduceMaxHalfInterval(workScalar, foldWork, curColNum);

    // brcMeta = score - max；Exp（pad 区 ≈0）
    ...
    // sum 归约：在 foldWork 上进行（pad 区 0），保留 brcMeta 的 exp 值
    Duplicate(foldWork, 0.0f, metaAlign);
    PipeBarrier<PIPE_V>();
    CopyCompactFloatsUb(foldWork, brcMeta, blockCount);
    ReduceSumHalfInterval(workScalar, foldWork, curColNum);
