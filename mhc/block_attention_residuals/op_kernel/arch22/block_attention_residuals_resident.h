...
private:
    ...
    __aicore__ inline void LoadResidentAndScores(int64_t tokenIdx)
    {
        for (uint32_t n = 0; n < blockCount_; ++n) {
            ...
            // UB→UB：Vector Copy 分段拷贝（Level0 mask-count 单指令上限：16bit 128 元素/rep，勿用 DataCopy）
            CopyCompactUb(dstRow, inLocal, hiddenSizeAlignBf16_);
            CastRowToFp32(vRow_, inLocal, hiddenSize_);
            PipeBarrier<PIPE_V>();
            FreeInRow(inLocal);
    ...
    __aicore__ inline void WeightedOutputResident()
    {
        ...
        for (uint32_t n = 0; n < blockCount_; ++n) {
            ...
            PipeBarrier<PIPE_V>();
            MulAddRowByBrcBlock(outFp32_, vRow_, metaBrc_[n * ELEM_PER_BLK_FP32], hiddenSize_, hiddenSizeAlignFp32_,
                                false);
            PipeBarrier<PIPE_V>();
        }
        SetMaskNorm();
