 ...
 * \brief TK-HSLICE：以 TK-RELOAD 为基础，对 H 切块（hiddenSizeChunk）逐块重复搬运 v，
 *        支持任意 H（H 上限仅受 GM 容量约束）。AIV 向量路径，无 GetValue/SetValue。
 *
 * 计算流程（每 token）：
...
public:
    __aicore__ inline BlockAttentionResidualsHSlice(TPipe *pipe, const BlockAttentionResidualsTilingData *tilingData)
    {
        ...
        hChunkAlignBf16_ = (hiddenSizeChunk_ + ELEM_PER_BLK_BF16 - 1U) / ELEM_PER_BLK_BF16 * ELEM_PER_BLK_BF16;
        hChunkAlignFp32_ = (hiddenSizeChunk_ + ELEM_PER_BLK_FP32 - 1U) / ELEM_PER_BLK_FP32 * ELEM_PER_BLK_FP32;
    }
...
private:
    __aicore__ inline void KahanSumUpdate(LocalTensor<float> &input, LocalTensor<float> sumList[2], int32_t &sumPos)
    {
        ...
        Sub(input, input, compensation, blockCount_); // y = x - c
        PipeBarrier<PIPE_V>();
        Add(compensation, input, sum, blockCount_); // t = sum + y
        PipeBarrier<PIPE_V>();
        Sub(sum, compensation, sum, blockCount_);
        PipeBarrier<PIPE_V>();
        Sub(sum, sum, input, blockCount_); // c = (t - sum) - y
        PipeBarrier<PIPE_V>();
        sumPos = 1 - sumPos;
    }
    ...
    __aicore__ inline void LoadScoreWeightChunk(int64_t hcOffset, uint32_t hcLen)
    {
        ...
        CastRowToFp32(vRow_, inLocal, hcLen);
        PipeBarrier<PIPE_V>();
        FreeInRow(inLocal);

        inLocal = CopyInRowSyncOff(normWeightGm_, hcOffset, hcLen);
        CastRowToFp32(outFp32_, inLocal, hcLen);
        PipeBarrier<PIPE_V>();
        FreeInRow(inLocal);

        Mul(scoreWeight_, vRow_, outFp32_, hcLen);
        PipeBarrier<PIPE_V>();
    }
    ...
    __aicore__ inline void ProcessRowScoreChunk(const LocalTensor<D_IN> &inLocal, uint32_t metaIdx, uint32_t hcLen)
    {
        CastRowToFp32(vRow_, inLocal, hcLen);
        PipeBarrier<PIPE_V>();
        Mul(outFp32_, vRow_, vRow_, hcLen);
        PipeBarrier<PIPE_V>();
        ReduceSumHalfInterval(vecMeta_[metaIdx], outFp32_, static_cast<int32_t>(hcLen));
        PipeBarrier<PIPE_V>();
        Mul(outFp32_, vRow_, scoreWeight_, hcLen);
        PipeBarrier<PIPE_V>();
        ReduceSumHalfInterval(metaSoftmax_[metaIdx], outFp32_, static_cast<int32_t>(hcLen));
        PipeBarrier<PIPE_V>();
    }
    ...
    __aicore__ inline void FinalizeScores(int64_t tokenIdx)
    {
        ...
        Muls(sumSqAcc, sumSqAcc, invHiddenSize_, blockCount_);
        PipeBarrier<PIPE_V>();
        Adds(sumSqAcc, sumSqAcc, normEps_, blockCount_);
        PipeBarrier<PIPE_V>();
        Sqrt(sumSqAcc, sumSqAcc, blockCount_);
        PipeBarrier<PIPE_V>();
        Duplicate(metaSoftmax_, 1.0f, metaAlign_);
        PipeBarrier<PIPE_V>();
        Div(sumSqAcc, metaSoftmax_, sumSqAcc, blockCount_); // sumSqAcc ← invNorm
        PipeBarrier<PIPE_V>();
        Mul(scoreAcc, scoreAcc, sumSqAcc, blockCount_);
        PipeBarrier<PIPE_V>();
        if (needBackward_) {
            LocalTensor<float> invUb = invQue_.AllocTensor<float>();
            CopyCompactFloatsUb(invUb, sumSqAcc, blockCount_);
            invQue_.EnQue(invUb);
    ...
    {
        LocalTensor<float> scoreAcc = GetScoreAcc();
        SoftmaxSmallVec(scoreAcc, blockCount_, metaAlign_, scalarLocal_, metaSoftmax_, metaBrc_);
        // 紧凑 prob → metaBrc[n*8]：零填充 staging 后单次 Brcb（repeat=ceil(B/8)）
        Duplicate(metaSoftmax_, 0.0f, metaAlign_);
        PipeBarrier<PIPE_V>();
        CopyCompactFloatsUb(metaSoftmax_, scoreAcc, blockCount_);
        const uint8_t brcRepeat = static_cast<uint8_t>((blockCount_ + ELEM_PER_BLK_FP32 - 1U) / ELEM_PER_BLK_FP32);
    ...
    __aicore__ inline void WriteProbsQue(int64_t tokenIdx)
    {
        ...
        CopyCompactFloatsUb(probsUb, scoreAcc, blockCount_);
    ...
    __aicore__ inline void ProcessRowWeighted(const LocalTensor<D_IN> &inLocal, uint32_t metaIdx, uint32_t hcLen)
    {
        ...
        PipeBarrier<PIPE_V>();
        MulAddRowByBrcBlock(outFp32_, vRow_, metaBrc_[metaIdx * ELEM_PER_BLK_FP32], hcLen, hChunkAlignFp32_, false);
        PipeBarrier<PIPE_V>();
    }
    ...
    __aicore__ inline void WeightedOutputSliced(int64_t tokenIdx)
    {
        for (int64_t hc = 0; hc < hChunks_; ++hc) {
            ...
            SetMaskCount();
            SetVectorMask<float, MaskMode::COUNTER>(hcLen);
            for (uint32_t n = 0; n < blockCount_; ++n) {
                LocalTensor<D_IN> inLocal = CopyInRowDeQue();
                if (n + 1U < blockCount_) {
                    CopyInRowEnqueueOff(GetLogicRowGm(n + 1U), GetLogicRowOffset(tokenIdx, n + 1U) + hcOffset, hcLen);
    ...
    __aicore__ inline void WriteHiddenStatesChunk(int64_t tokenIdx, int64_t hcOffset, uint32_t hcLen)
    {
        ...
        CastFp32ToRow(outLocal, outFp32_, hcLen);
