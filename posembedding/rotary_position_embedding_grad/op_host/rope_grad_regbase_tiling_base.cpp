...
ge::graphStatus RopeGradRegBaseTilingClass::CheckNullptr()
{
    ...
    for (int64_t i = 0; i <= checkOutputIndexRange; i++) {
        auto desc = context_->GetInputDesc(i);
        OP_CHECK_IF(desc == nullptr, OP_LOGE(context_, "output %ld desc is nullptr.", i), return ge::GRAPH_FAILED);
        auto shape = context_->GetInputShape(i);
        OP_CHECK_IF(shape == nullptr, OP_LOGE(context_, "output %ld shape is nullptr.", i), return ge::GRAPH_FAILED);
    }
    return ge::GRAPH_SUCCESS;
}
    ...
    for (size_t i = 0; i < shape.GetDimNum(); i++) {
        ...
        if (shape.GetDim(i) < 0) {
            ...
            std::string reasonMsg =
                "The shape of input " + inputNames[idx] + " can not be an invalid tensor with a negative dimension";
...
ge::graphStatus RopeGradRegBaseTilingClass::CheckOutPutShapeAllPositive(const int64_t idx) const
{
    ...
    for (size_t i = 0; i < shape.GetDimNum(); i++) {
        ...
        if (shape.GetDim(i) < 0) {
            std::string shapeMsg = ToString(shape);
            std::string reasonMsg =
                "The shape of output " + outputNames[idx] + " can not be an invalid tensor with a negative dimension";
...
ge::graphStatus RopeGradRegBaseTilingClass::CheckShapeAllPositive() const
{
    ...
    OP_CHECK_IF(CheckInPutShapeAllPositive(DY_INDEX) != ge::GRAPH_SUCCESS, OP_LOGE(context_, "dy has negative shape."),
                return ge::GRAPH_FAILED);
    ...
    OP_CHECK_IF(CheckOutPutShapeAllPositive(DX_INDEX) != ge::GRAPH_SUCCESS, OP_LOGE(context_, "dx has negative shape."),
                return ge::GRAPH_FAILED);
    if (context_->GetInputShape(X_INDEX) != nullptr) {
        OP_CHECK_IF(CheckInPutShapeAllPositive(X_INDEX) != ge::GRAPH_SUCCESS,
                    OP_LOGE(context_, "x has negative shape."), return ge::GRAPH_FAILED);
        OP_CHECK_IF(CheckOutPutShapeAllPositive(DCOS_INDEX) != ge::GRAPH_SUCCESS,
                    OP_LOGE(context_, "dcos has negative shape."), return ge::GRAPH_FAILED);
        OP_CHECK_IF(CheckOutPutShapeAllPositive(DSIN_INDEX) != ge::GRAPH_SUCCESS,
...
bool RopeGradRegBaseTilingClass::Is3dBsdBroadcastLayout(const gert::Shape &xShape, const gert::Shape &cosShape) const
{
    ...
    return xShape.GetDimNum() == DIM_NUM_TND && cosShape.GetDimNum() == DIM_NUM_TND && xShape.GetDim(DIM_1) != 1 &&
           cosShape.GetDim(DIM_0) == 1 && cosShape.GetDim(DIM_1) == xShape.GetDim(DIM_1) &&
           cosShape.GetDim(DIM_2) == xShape.GetDim(DIM_2);
}
...
ge::graphStatus RopeGradRegBaseTilingClass::CheckShape()
{
    ...
    OP_CHECK_IF(CheckRotaryModeShapeRelation(dyLastDim) != ge::GRAPH_SUCCESS,
    ...
    if (rotaryMode_ == RotaryPosEmbeddingMode::HALF || rotaryMode_ == RotaryPosEmbeddingMode::INTERLEAVE ||
        ...
        if (d % HALF_INTERLEAVE_MODE_COEF != 0) {
            ...
            std::string reasonMsg = "The D axis of input dy should be divisible by " +
                                    ...
                                    " when the attribute mode is half, interleave or interleave-half, "
                                    "where D refers to the last dim";
            OP_LOGE_FOR_INVALID_SHAPE_WITH_REASON(context_->GetNodeName(), "dy", shapeMsg.c_str(), reasonMsg.c_str());
            return ge::GRAPH_FAILED;
        }
        ...
        if (d % QUARTER_MODE_COEF != 0) {
            std::string shapeMsg = ToString(dyShape);
            std::string reasonMsg = "The D axis of input dy should be divisible by " +
                                    std::to_string(QUARTER_MODE_COEF) +
...
ge::graphStatus RopeGradRegBaseTilingClass::GetShapeAttrsInfo()
{
    ...
    OP_CHECK_IF(JudgeLayoutByShape(dyShape_, cosShape_) != ge::GRAPH_SUCCESS,
...
ge::graphStatus RopeGradRegBaseTilingClass::GetInputParam(Ops::Base::ReduceOpInputParam &opInput, uint32_t inputIdx,
                                                          uint32_t axesIdx)
{
    // set opInput dtype
...
ge::graphStatus RopeGradRegBaseTilingClass::InitTilingData()
{
    ...
    OP_CHECK_IF((memset_s(tilingData_, sizeof(RopeGradTilingData), 0, sizeof(RopeGradTilingData)) != EOK),
...
ge::graphStatus RopeGradRegBaseTilingClass::GetReduceOpCompileInfo(Ops::Base::ReduceOpCompileInfo *compileInfo)
{
    ...
    OP_CHECK_IF(ubSize <= static_cast<uint64_t>(Ops::Base::CACHE_BUF_SIZE),
                OP_LOGE(context_->GetNodeName(), "ReduceOp GetHardwareInfo Failed, ubSize:%lu, at least:%ld.",
    ...
    OP_CHECK_IF(
        ...
        OP_LOGE(context_->GetNodeName(), "ReduceOp GetHardwareInfo Failed, ubBlockSize:%lu.", compileInfo->ubBlockSize),
        return ge::GRAPH_FAILED);

    compileInfo->vRegSize = Ops::Base::GetVRegSize(context_);
    OP_CHECK_IF(
        compileInfo->vRegSize == 0,
        OP_LOGE(context_->GetNodeName(), "ReduceOp GetHardwareInfo Failed, vRegSize:%lu.", compileInfo->vRegSize),
        return ge::GRAPH_FAILED);
...
ge::graphStatus RopeGradRegBaseTilingClass::TilingReduce()
{
    ...
    OP_CHECK_IF((GetInputParam(opInput, 0, 1) == ge::GRAPH_FAILED),
                OP_LOGE(context_->GetNodeName(), "ReduceOp get dy input param failed"), return ge::GRAPH_FAILED);
    ge::graphStatus status;
    if (ge::GetSizeByDataType(opInput.inputDtype) == sizeof(float)) {
        status = Ops::Base::Tiling4ReduceOp<
            RotaryPositionEmbeddingGrad::RotaryPositionEmbeddingGradDag<float, float>::OpDag>(
            context_, opInput, key_, &opInfo, &tilingData_->reduceTiling);
    } else {
        status = Ops::Base::Tiling4ReduceOp<
            RotaryPositionEmbeddingGrad::RotaryPositionEmbeddingGradDag<Ops::Base::half, float>::OpDag>(
            context_, opInput, key_, &opInfo, &tilingData_->reduceTiling);
    }
