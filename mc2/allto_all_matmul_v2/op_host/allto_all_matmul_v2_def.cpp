...
/*!
 * \file allto_all_matmul_v2_def.cpp
 * \brief OpDef definition for AlltoAllMatmulV2 (MX-quant FP8 only, apace UDMA path)
 */

#include <register/op_def_registry.h>

namespace ops {
class AlltoAllMatmulV2 : public OpDef {
public:
    explicit AlltoAllMatmulV2(const char *name)
        : OpDef(name)
    {
        this->Input("context")
            .ParamType(REQUIRED)
        ...
        this->Attr("hccl_buffer_size").AttrType(REQUIRED).Int(0);
        this->Attr("y_dtype").AttrType(OPTIONAL).Int(static_cast<int64_t>(ge::DT_FLOAT));
        ...
        aicoreConfig950.DynamicCompileStaticFlag(true)
            ...
            .ExtendCfgInfo("opFile.value", "allto_all_matmul_v2_apt");
        this->AICore().AddConfig("ascend950", aicoreConfig950);
    }
};

OP_ADD(AlltoAllMatmulV2);
} // namespace ops
