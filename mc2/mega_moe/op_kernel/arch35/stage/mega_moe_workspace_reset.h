...
__aicore__ inline int64_t CalcResetFlagElementCount(const MegaMoeTilingData *tilingData)
{
    ...
    int64_t flagElementCount = moeExpertCount * (activationFlagSlotsPerExpert + waveFlagSlotsPerExpert +
    ...
    if (useGroupSyncCounters) {
        flagElementCount +=
            static_cast<int64_t>(tilingData->combineSyncSlotCountPerExpert) * moeExpertCount * INT_CACHELINE;
    }
...
__aicore__ inline void ResetSyncStatus(const DispatchPrepareConfig &context, const Params &params,
                                       const ResetWorkspaceConfig &config, LocalTensor<int32_t> &resetTensor)
{
    ...
    if constexpr (TopkWeightsPrefetch) {
        int32_t statusElementCount = (static_cast<int32_t>(context.common.moeExpertPerRank) *
