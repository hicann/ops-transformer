...
CutResult UnQuantMatmulAllReduceTiling310::GetTilingResult()
{
    ...
    SocVersion_2201 socVer = SocVersion_2201::SOC910_B;
    ...
    if ((commQuantScaleOne != nullptr) && (commQuantScaleTwo != nullptr)) { // low-bit comm
        ...
        MMPlusQuantAllReduce quantAllReduceHcclTiling(args_, args_.rankDim, KernelType::ALL_REDUCE, npuArch_);
        quantAllReduceHcclTiling.GetTiling();
        mCutAllreduceRet = quantAllReduceHcclTiling.tilingM_.cutRes;
    } else {
        MMPlusAllReduce allReduceHcclTiling(args_, args_.rankDim, KernelType::ALL_REDUCE, npuArch_, isPerBlock_);
