...
KERNEL_SUBSTR = "quant_matmul_reduce_scatter"
GROUP_THRESHOLD_S = 5
OUTLIER_FACTOR = 1.2
...
def report_case(prof_base, case_dirs, case_idx, mkn_str=None):
    ...
    if not all_devices:
        print(f"  [Case {case_idx}] No quant_matmul_reduce_scatter rows found.\n")
        return None
...
def main():
    ...
    parser.add_argument(
        "--mkn", type=str, default=None, help="M K N string, e.g. '256 512 512'"
    )
    parser.add_argument(
        "--prof-dir",
    ...
    print("=" * 64)
    print(
        "QuantMatmulReduceScatter (ubmem) — Per-Card Latency (avg, outliers > 2x min excluded)"
    )
    print("=" * 64)
