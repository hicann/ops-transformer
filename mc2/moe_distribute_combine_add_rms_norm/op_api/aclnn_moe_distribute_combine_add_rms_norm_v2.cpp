    ...
    aclTensor *xOut, uint64_t *workspaceSize, aclOpExecutor **executor)
{
    ...
    aclnnStatus ret = aclnnMoeDistributeCombineAddRmsNormGetWorkspaceSizeBase(
        ...
        constExpertVOptional, groupEp, epWorldSize, epRankId, moeExpertNum, groupTp, tpWorldSize, tpRankId,
        expertShardType, sharedExpertNum, sharedExpertRankNum, globalBs, outDtype, commQuantMode, groupListType,
        commAlg, normEps, zeroExpertNum, copyExpertNum, constExpertNum, yOut, rstdOut, xOut, workspaceSize, executor);
    return ret;
}
