...
* @li expand_x: A tensor. The characteristics of the extended token based on expandIds. Support dtype: bfloat16,
dimension must be 2, Support Shape: (A * world_size, H), support format: ND.
* @li expert_ids: A tensor. The topK expert index for each token. Support dtype: int32, dimension must be 2, Support
Shape: (BS, K), support format: ND.
* @li assist_info_for_combine: A tensor. An output of MoeDistributeDispatch. Support dtype: int32, dimension must be 1,
Support Shape: (A * 128), support format: ND.
* @li ep_send_counts: A tensor. An output of MoeDistributeDispatch. Support dtype: int32, Support Shape: (expert_nums +
2 * globalBs * K * server_num, ), support format: ND.
* @li expert_scales: A tensor. The weight of the topK experts for each token. Support dtype: float32, Support Shape:
(BS, K), support format: ND.
* @li residual_x: A tensor. The right matrix of the add in AddRmsNorm. Support dtype: bfloat16, Support Shape: (BS, 1,
H), support format: ND.
* @li gamma: A tensor. An input of addRmsNorm. Support dtype: bfloat16, Support Shape: (H), support format: ND.
* @li tp_send_counts: A tensor. The output tpRecvCounts in MoeDistributeDispatch. Support dtype: int32, support format:
ND.
* @li x_active_mask: An optional tensor. Whether the token participates in the communication. True: participate, False:
do not participate. Support dtype: bool, support format: ND.
...
* @li expand_scales: A tensor. An output of MoeDistributeDispatch. Support dtype: float32, Support Shape: (A, ), support
format: ND.
* @li shared_expert_x: A tensor. Share the tokens calculated by experts. Support dtype: bfloat16, support format: ND.

* @par Attributes
* @li group_ep: Input ep comm group name, ep means experts parallelism, dtype: String. Support string length [1, 128),
could not bee the same with group_tp.
* @li ep_world_size: Input ep comm world size, dtype: Int64. Support value range: [1, 384].
* @li ep_rank_id: Input ep comm rank Id, dtype: Int64. Support value range: [0, ep_world_size), could not be duplicated
within the same EP domain.
* @li moe_expert_num: Input moe expert num, dtype: Int64. Support value range: [0, 512).
* @li group_tp: Input tp comm group name, tp means tensor parallelism, dtype: String. Support string length [1, 128),
could not bee the same with group_ep. Default value: "".
* @li tp_world_size: Input tp comm world size, dtype: Int64. Support value range: [0, 2]. Default value: 0.
* @li tp_rank_id: Input tp comm rank Id, dtype: Int64. Support value range: [0, tp_world_size), could not be duplicated
within the same TP domain. Default value: 0.
...
* @li shared_expert_rank_num: Input shared expert rank num, dtype: Int64. Support value range: [0, ep_world_size - 1).
Default value: 0.
* @li global_bs: Input global batch size, dtype: Int64. When bs is the same on all the ranks, globalBs = bs *
ep_world_size or globalBs = 0; when bs is not the same on all the ranks, globalBs = maxBs * ep_world_size. Default
value: 0.
* @li out_dtype: Dtype of output, 0 for bfloat16, 1 for float16, dtype: Int64. Default value: 0. It's not supported yet.
* @li comm_quant_mode: Communication quantization mode, 1 for enable, 0 for disable, dtype: Int64. Default value: 0.
...
* @li y: A tensor. Result of (combine - Add) + Add + RmsNorm. Support dtype: bfloat16,  Support Shape: (BS, 1, H),
support format: ND.
* @li rstd: A tensor. Result of standard deviation. Support dtype: float32,  Support Shape: (BS, 1, 1), support format:
ND.
* @li x: A tensor. Result of (combine - Add) + Add. Support dtype: bfloat16,  Support Shape: (BS, 1, H), support format:
ND.
*/
