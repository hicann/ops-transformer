...
int LaunchOneThreadAllGatherMmV2(Args &args)
{
    ...
    CHECK_RET(ret == ACL_SUCCESS, return ret);
    ret = CreateAclTensor(gatherOutHostData, gatherOutShape, &gatherOutDeviceAddr, aclDataType::ACL_INT8, &gatherOut);
    CHECK_RET(ret == ACL_SUCCESS, return ret);

    // 调用第一阶段接口
    ret = aclnnAllGatherMatmulV2GetWorkspaceSize(x1, x2, bias, x1Scale, x2Scale, quantScale, blockSize, hcomName,
                                                 gatherIndex, commTurn, streamMode, groupSize, "aiv", out, gatherOut,
